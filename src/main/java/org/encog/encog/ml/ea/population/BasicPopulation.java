...
public class BasicPopulation extends BasicML implements Population,
    ...
    public List<Genome> flatten() {
        ...
        this.species.stream().forEach((species) -> result.addAll(species.getMembers()));
        return result;
    }
    ...
    public void purgeInvalidGenomes() {
        ...
        while (speciesNum < getSpecies().size()) {
            final Species species = getSpecies().get(speciesNum);

            int genomeNum = 0;
            while (genomeNum < species.getMembers().size()) {
                final Genome genome = species.getMembers().get(genomeNum);
