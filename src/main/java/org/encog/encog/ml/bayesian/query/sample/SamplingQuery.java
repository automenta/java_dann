...
public class SamplingQuery extends BasicQuery implements Serializable {
     ...
     */
    public SamplingQuery(final BayesianNetwork theNetwork) {
        super(theNetwork);
    }
    ...
    private void randomizeEvents(final EventState eventState) {
        ...
        eventState.getEvent()
                .getChildren().stream().forEach((childEvent) -> randomizeEvents(getEventState(childEvent)));
    }
    ...
    public void execute() {
        ...
        for (int i = 0; i < this.sampleSize; i++) {
            ...
            int uncalculated;
            do {
                getEvents().values().stream().forEach(this::randomizeEvents);
