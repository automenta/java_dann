...
public class CalculateScheduleTimes {
    ...
    public void forward(final ActionNode node) {
        ...
        node.getConnections().stream().forEach((edge) -> forward((ActionNode) edge.getDestination()));
    }
    ...
    public void backward(final ActionNode node) {
        ...
        node.getBackConnections().stream().forEach((edge) -> backward((ActionNode) edge.getSource()));
    }

    public void calculate(final ScheduleGraph graph) {
        // forward pass
        graph.getStartNode().setEarliestStartTime(0);
        graph.getStartNode().getConnections().stream().forEach((edge) -> forward((ActionNode) edge.getDestination()));

        // backward
        graph.getFinishNode().setLatestStartTime(
                graph.getFinishNode().getEarliestStartTime());
        graph.getFinishNode().getBackConnections().stream().forEach((edge) -> backward((ActionNode) edge.getSource()));
    }
}
