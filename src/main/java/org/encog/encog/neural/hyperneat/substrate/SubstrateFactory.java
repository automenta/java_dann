...
public class SubstrateFactory {
     ...
     */
    public static Substrate factorSandwichSubstrate(final int inputEdgeSize,
                                                    final int outputEdgeSize) {
        final Substrate result = new Substrate(3);

        final double inputTick = 2.0 / inputEdgeSize;
        final double outputTick = 2.0 / inputEdgeSize;
        final double inputOrig = -1.0 + (inputTick / 2.0);
        final double outputOrig = -1.0 + (inputTick / 2.0);

        // create the input layer

        for (int row = 0; row < inputEdgeSize; row++) {
            for (int col = 0; col < inputEdgeSize; col++) {
                final SubstrateNode inputNode = result.createInputNode();
        ...
        for (int orow = 0; orow < outputEdgeSize; orow++) {
            for (int ocol = 0; ocol < outputEdgeSize; ocol++) {
                final SubstrateNode outputNode = result.createOutputNode();
                ...
                result.getInputNodes().stream().forEach((inputNode) -> result.createLink(inputNode, outputNode));
            }
        }
