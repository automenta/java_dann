...
public class PersistHMM implements EncogPersistor {
	...
	public final Object read(final InputStream is) {
		int states = 0;
		double pi[] = null;
		SimpleRealMatrix transitionProbability = null;
		Map<String, String> properties = null;
		final List<StateDistribution> distributions = new ArrayList<StateDistribution>();
		...
		while ((section = in.readNextSection()) != null) {
			...
			if (section.getSectionName().equals("HMM")
				...
				if (params.containsKey(HiddenMarkovModel.TAG_ITEMS)) {
					EncogFileSection.parseIntArray(params,
							HiddenMarkovModel.TAG_ITEMS);
				}
				pi = section.parseDoubleArray(params, HiddenMarkovModel.TAG_PI);
				transitionProbability = EncogFileSection.parseMatrix(params,
		...
		result.setPi(pi);
		int index = 0;
		for (final StateDistribution dist : distributions) {
	...
	public final void save(final OutputStream os, final Object obj) {
		...
		out.writeProperty(HiddenMarkovModel.TAG_TRANSITION,
				new Matrix(net.getTransitionProbability()));

		for (int i = 0; i < net.getStateCount(); i++) {
			out.addSubSection("DISTRIBUTION-" + i);
			final StateDistribution sd = net.getStateDistribution(i);
			out.writeProperty(HiddenMarkovModel.TAG_DIST_TYPE, sd.getClass()
					.getSimpleName());

			if (sd instanceof ContinousDistribution) {
				final ContinousDistribution cDist = (ContinousDistribution) sd;
				out.writeProperty(HiddenMarkovModel.TAG_MEAN, cDist.getMean());
				out.writeProperty(HiddenMarkovModel.TAG_COVARIANCE,
						cDist.getCovariance());

			} else if (sd instanceof DiscreteDistribution) {
				final DiscreteDistribution dDist = (DiscreteDistribution) sd;
