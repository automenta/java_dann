...
public abstract class AbstractInputNeuron extends AbstractNeuron implements
        InputNeuron {
    ...
    public void setInput(final double inputToSet) {
        if (Math.abs(inputToSet) > 1.0)
            throw new IllegalArgumentException(
                    "InputToSet must be between -1 and +1");
    ...
    public void tick() {
        getBrain().getTraversableEdges(
                this).stream().forEach((current) -> current.setInput(input));
    }
}
