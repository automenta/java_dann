 ...
 * corelate to a specific dataset pattern.<br/>
 * <br/>
...
public abstract class AbstractLocalBrain<IN extends InputNeuron, ON extends OutputNeuron, N extends Neuron, S extends Synapse<N>>
    ...
    protected boolean add(final S newSynapse) {
        ...
        if (!this.neurons.containsAll(newSynapse.getNodes()))
            throw new IllegalArgumentException(
                    "newSynapse has a node as an end point that is not part of the graph");
    ...
    protected boolean add(final Collection<? extends N> newNeurons) {
        ...
        newNeurons.stream().map((final N newNeuron) -> {
            this.outMap.put(newNeuron, new HashSet<>());
            return newNeuron;
        }).map((newNeuron) -> {
            this.inMap.put(newNeuron, new HashSet<>());
            ...
            if (newNeuron instanceof OutputNeuron)
                this.outputNeurons.add((ON) newNeuron);
            return newNeuron;
        }).filter((newNeuron) -> (newNeuron instanceof InputNeuron)).forEach((newNeuron) -> this.inputNeurons.add((IN) newNeuron));
    ...
    /*
     * protected boolean connect(final N source, final N destination, final
     * double initialWeight) { if( source == null ) throw new
     * IllegalArgumentException("source can not be null"); if( destination ==
     * null ) throw new IllegalArgumentException("destination can not be null");
     * if( !this.neurons.contains(source) ) throw new
     * IllegalArgumentException("source is not a member of this graph"); if(
     * !this.neurons.contains(destination) ) throw new
     * IllegalArgumentException("destination is not a member of this graph");
     *
     * return this.add((S)(new SimpleSynapse(source, destination,
     * initialWeight))); }
     */
    protected boolean connect(final S synapse) {
        if (!this.neurons.containsAll(synapse.getNodes()))
            throw new IllegalArgumentException(
                    "the synapse contains nodes not in this brain");

        return this.add(synapse);
    }

	/*
     * protected boolean disconnect(final Neuron source, final Neuron
	 * destination) { if( source == null ) throw new
	 * IllegalArgumentException("source can not be null"); if( destination ==
	 * null ) throw new IllegalArgumentException("destination can not be null");
	 * if( !this.neurons.contains(source) ) throw new
	 * IllegalArgumentException("source is not a member of this graph"); if(
    ...
    protected boolean remove(final S removeSynapse) {
        ...
        if (this.synapses.remove(removeSynapse)) {
            if (this.outMap.containsKey(removeSynapse.getSourceNode()))
                this.outMap.get(removeSynapse.getSourceNode()).remove(
                        removeSynapse);
            if (this.inMap.containsKey(removeSynapse.getDestinationNode()))
                this.inMap.get(removeSynapse.getDestinationNode()).remove(
                        removeSynapse);
            return true;
        }
        return false;
    ...
    public List<N> getAdjacentNodes(final N node) {
        ...
        nodeSynapses.stream().forEach((nodeSynapse) -> neighbors.add((nodeSynapse.getLeftNode().equals(node) ? nodeSynapse
