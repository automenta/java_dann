    ...
    private void calculateNeuronGradient(final FreeformNeuron toNeuron) {
        ...
        if (toNeuron.getInputSummation() != null) {
            ...
            toNeuron
                ...
                return connection;
            }).map(FreeformConnection::getSource).forEach((fromNeuron) -> {
                double sum = 0;
                sum = fromNeuron
            ...
            toNeuron
                    .getInputSummation().list().stream().map(FreeformConnection::getSource).forEach(this::calculateNeuronGradient);
