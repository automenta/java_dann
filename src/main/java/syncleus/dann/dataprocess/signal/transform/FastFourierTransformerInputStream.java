...
public class FastFourierTransformerInputStream extends InputStream {
	...
	public FastFourierTransformerInputStream(final InputStream inputStream,
			final FastFourierTransformer ourTransformer, final int ourInterval)
			throws IOException {
		if (inputStream == null)
		...
		if (ourInterval <= 0)
			throw new IllegalArgumentException(
					"ourInterval must be greater than 0");
	...
	public int readTransform(final DiscreteFourierTransform[] b, final int off,
			final int len) throws IOException {
		if (b == null)
			throw new IllegalArgumentException("b must not be null");
		if (off > b.length)
			throw new IllegalArgumentException(
					"off is greater than length of b");
		if (off < 0)
			throw new IllegalArgumentException(
					"off must be greater than or equal to 0");
		if (len < 0)
			throw new IllegalArgumentException(
					"len must be greater than or equal to 0");
		if ((off + len) > b.length)
			throw new IllegalArgumentException(
					"(off + len) is greater than length of b");
		...
		if (this.transformsAvailable() <= 1) {
			...
			if (bufferSpace > len)
				bufferSpace = len;
			final int transformCount = (bufferSpace < this
					.transformsAvailable() ? bufferSpace : this
					.transformsAvailable());
	...
	public int readTransform(final DiscreteFourierTransform[] b)
			throws IOException {
		if (b == null)
			throw new IllegalArgumentException("b must not be null");
		...
		if (this.transformsAvailable() <= 1) {
			...
			final int transformCount = (b.length < this.transformsAvailable() ? b.length
					: this.transformsAvailable());
	...
	public DiscreteFourierTransform readTransform() throws IOException {
		...
		if (this.buffer == null)
			...
			signal = Arrays
					.copyOf(this.buffer, this.transformer.getBlockSize());

		if ((this.buffer == null)
				|| (this.buffer.length <= this.transformer.getBlockSize())) {
			// signal = Arrays.copyOf(this.buffer,
			// this.transformer.getBlockSize());
			for (int signalIndex = (this.buffer == null ? 0
					: this.buffer.length); signalIndex < signal.length; signalIndex++)
		...
		if (this.buffer != null) {
			if (this.buffer.length <= signalsToRemove) {
				...
				this.buffer = Arrays.copyOfRange(this.buffer, signalsToRemove,
						this.buffer.length);
				signalsToRemove = 0;
			}
		}
