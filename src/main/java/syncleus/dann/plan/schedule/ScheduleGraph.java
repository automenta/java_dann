...
public class ScheduleGraph extends PathGraph {
    ...
    public ActionNode addChild(final ActionNode parent, final String name,
                               final double duration) {
        final ActionNode newNode = new ActionNode(name, duration);
    ...
    public void dumpNode(final StringBuilder result, final ActionNode node,
        ...
        if (!visited.containsKey(node)) {
            ...
            result.append('\n');
            node.getConnections().stream().forEach((edge) -> dumpNode(result, (ActionNode) edge.getDestinationNode(), visited));
        }
    }
