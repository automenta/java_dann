...
public class NavigationCell {
     ...
     */
    private NavigationCell[] links = new NavigationCell[3];
     ...
     * @return
     */
    boolean requestLink(Vector3f pointA, Vector3f pointB, NavigationCell caller, float epsilon) {
     ...
     * @param Caller
     */
    private void setLink(int Side, NavigationCell Caller) {
        links[Side] = Caller;
    }
    ...
    NavigationCell getLink(int side) {
        return (links[side]);
    }
...
//					&& end != Line2D.POINT_CLASSIFICATION.ON_LINE) {
     ...
     * @return
     */
    boolean queryForPath(Heap heap, NavigationCell caller, float arrivalCost) {
        if (sessionID != heap.getSessionID()) {
...
//        capsule.write(wallDistances, "distances", null);
    ...
    public void read(JmeImporter e) throws IOException {
        ...
        for (int i = 0; i < savLinks.length; i++){
            links[i] = (NavigationCell) savLinks[i];
        }
...
//        wallDistances = capsule.readFloatArray("distances", new float[3]);
    ...
    void checkAndLink(NavigationCell other, float epsilon) {
        if (getLink(NavigationCell.SIDE_AB) == null
                && other.requestLink(getVertex(0), getVertex(1), this, epsilon)) {
            setLink(NavigationCell.SIDE_AB, other);
        } else if (getLink(NavigationCell.SIDE_BC) == null
                && other.requestLink(getVertex(1), getVertex(2), this, epsilon)) {
            setLink(NavigationCell.SIDE_BC, other);
        } else if (getLink(NavigationCell.SIDE_CA) == null
                && other.requestLink(getVertex(2), getVertex(0), this, epsilon)) {
            setLink(NavigationCell.SIDE_CA, other);
        }
    }

    void unLink(NavigationCell c) {
        if (c == links[0]) {
            links[0] = null;
            c.unLink(this);
        } else if (c == links[1]) {
