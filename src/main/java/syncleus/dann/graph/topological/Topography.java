...
public final class Topography {
    ...
    /**
     * Gets the degree of a given node. The degree of a node is how many
     * adjacent edges link to this node, as opposed to how many nodes this node
     * links to. A typical adjacent edge will connect once to this edge and to
     * one or more other nodes, counting once towards the degree. However, loop
     * edges can connect twice or more to this node counting multiple times.
     *
     ...
     */
    public static <N, E extends Edge<N>> int getDegree(final Graph<N, E> graph,
                                                       final N node) {
        ...
        if (graph instanceof BidirectedGraph) {
            final Set<BidirectedEdge<N>> adjacentEdges = ((BidirectedGraph<N, BidirectedEdge<N>>) graph)
    ...
    /**
     * Determines whether this graph is strongly connected. A graph is strongly
     * connected if, and only if every node is strongly connected to every other
     * node.
     *
     ...
     */
    public static <N, E extends Edge<N>> boolean isStronglyConnected(
            final Graph<N, E> graph) {
        if (graph instanceof StrongConnectivityOptimizedGraph) {
            try {
                return ((StrongConnectivityOptimizedGraph) graph)
                        .isStronglyConnected();
            } catch (final UnsupportedOperationException caught) {
                // if it is not supported, lets handle it as if it was not
                // optimized
    ...
    /**
     * Determines whether this graph is weakly connected. A graph is weakly
     * connected if there is only one node or every node is weakly connected to
     * every other node.
     *
     * @return If this graph is weakly connected
     * @see com.syncleus.dann.graph.topological.Topography#isWeaklyConnected(Graph)
     */
    public static <N, E extends Edge<N>> boolean isWeaklyConnected(
            final Graph<N, E> graph) {
        if (graph instanceof WeakConnectivityOptimizedGraph) {
            try {
                return ((WeakConnectivityOptimizedGraph) graph)
                        .isWeaklyConnected();
            } catch (final UnsupportedOperationException caught) {
                // if it is not supported, lets handle it as if it was not
                // optimized
    ...
    /**
     * Determines whether two nodes are weakly connected. Nodes are weakly
     * connected if there is some path from the left node to the right node,
     * across any adjacent nodes. Assume that the hypothetical
     * <p/>
     * <pre>
     * A -&gt; B
     * </pre>
     * <p/>
     * graph is directional. This graph would have a strong connection from A to
     * B, but only a weak connection from B to A. This graph would, therefore,
     * be weakly connected overall, but not strongly connected overall.
     *
     ...
     */
    public static <N, E extends Edge<N>> boolean isWeaklyConnected(
            final Graph<N, E> graph, final N leftNode, final N rightNode) {
        if (graph instanceof WeakConnectivityOptimizedGraph) {
            try {
                return ((WeakConnectivityOptimizedGraph) graph)
                        .isWeaklyConnected(leftNode, rightNode);
            } catch (final UnsupportedOperationException caught) {
                // if it is not supported, lets handle it as if it was not
                // optimized
            }
        ...
        final Set<N> toVisit = new HashSet<>();
    ...
    /**
     * Determines if two nodes are strongly connected. Nodes are strongly
     * connected if there is some path from the left node to the right node,
     * across any traversable nodes. See <code>isWeaklyConnected(N,N)</code> for
     * an example of weak versus strong connections.
     *
    ...
    public static <N, E extends Edge<N>> boolean isStronglyConnected(
        ...
        if (graph instanceof StrongConnectivityOptimizedGraph) {
            try {
                return ((StrongConnectivityOptimizedGraph) graph)
                        .isStronglyConnected(leftNode, rightNode);
            } catch (final UnsupportedOperationException caught) {
                // if it is not supported, lets handle it as if it was not
                // optimized
            }
    ...
    /**
     * Gets the set of maximally-connected components from a graph. The
     * maximally-connected components are those with the most connections to
     * other nodes. NOTE: This method currently returns null.
     *
     * @return <code>null</code>
     * @see com.syncleus.dann.graph.topological.Topography#getMaximallyConnectedComponents(Graph)
     */
    public static <N, E extends Edge<N>> Set<Graph<N, E>> getMaximallyConnectedComponents(
            final Graph<N, E> graph) {
        if (graph instanceof WeakConnectivityOptimizedGraph) {
            try {
                return ((WeakConnectivityOptimizedGraph) graph)
                        .getMaximallyConnectedComponents();
            } catch (final UnsupportedOperationException caught) {
                // if it is not supported, lets handle it as if it was not
                // optimized
            }
    ...
    /**
     * Determines whether the given graph is a maximal subgraph of the current
     * graph. A subgraph is maximal if none of the edges of the parent graph not
     * in the subgraph connect to any nodes in the subgraph.
     *
     * @param subGraph A subgraph of this graph to be checked if maximally connected.
     * @return Whether this is a maximal subgraph
     * @see com.syncleus.dann.graph.topological.Topography#isMaximalSubgraph(Graph,
     * Graph)
     */
    public static <N, E extends Edge<N>> boolean isMaximalSubgraph(
            final Graph<N, E> graph, final Graph<N, E> subGraph) {
        if (graph instanceof WeakConnectivityOptimizedGraph) {
            try {
                return ((WeakConnectivityOptimizedGraph) graph)
                        .isMaximalSubgraph(subGraph);
            } catch (final UnsupportedOperationException caught) {
                // if it is not supported, lets handle it as if it was not
                // optimized
            }
    ...
    public static <N, E extends Edge<N>> boolean isCut(final Graph<N, E> graph,
                                                       final Set<N> cutNodes, final Set<E> cutEdges) {
        if (graph instanceof WeakConnectivityOptimizedGraph) {
            try {
                return ((WeakConnectivityOptimizedGraph) graph).isCut(cutNodes,
                        cutEdges);
            } catch (final UnsupportedOperationException caught) {
                // if it is not supported, lets handle it as if it was not
                // optimized
        ...
        return Topography.isStronglyConnected(Topography.deleteFromGraph(graph,
                cutNodes, cutEdges));
    }

    public static <N, E extends Edge<N>> boolean isCut(final Graph<N, E> graph,
                                                       final Set<N> cutNodes, final Set<E> cutEdges, final N begin,
                                                       final N end) {
        if (graph instanceof WeakConnectivityOptimizedGraph) {
            try {
                return ((WeakConnectivityOptimizedGraph) graph).isCut(cutNodes,
        ...
        return Topography.isStronglyConnected(
                Topography.deleteFromGraph(graph, cutNodes, cutEdges), begin,
                end);
    }

    public static <N, E extends Edge<N>> boolean isCut(final Graph<N, E> graph,
                                                       final Set<E> cutEdges) {
        if (graph instanceof WeakConnectivityOptimizedGraph) {
            try {
    ...
    public static <N, E extends Edge<N>> boolean isCut(final Graph<N, E> graph,
        ...
        return Topography.isCut(graph, Collections.singleton(node),
    ...
    public static <N, E extends Edge<N>> boolean isCut(final Graph<N, E> graph,
        ...
        return Topography.isCut(graph, Collections.<N>emptySet(),
                Collections.singleton(edge));
    }

    public static <N, E extends Edge<N>> boolean isCut(final Graph<N, E> graph,
                                                       final Set<E> cutEdges, final N begin, final N end) {
        if (graph instanceof WeakConnectivityOptimizedGraph) {
            try {
                return ((WeakConnectivityOptimizedGraph) graph).isCut(cutEdges,
                        begin, end);
    ...
    public static <N, E extends Edge<N>> boolean isCut(final Graph<N, E> graph,
                                                       final N node, final N begin, final N end) {
        if (graph instanceof WeakConnectivityOptimizedGraph) {
            try {
                return ((WeakConnectivityOptimizedGraph) graph).isCut(node,
                        begin, end);
        ...
        return Topography.isCut(graph, Collections.singleton(node),
                Collections.<E>emptySet(), begin, end);
    }

    public static <N, E extends Edge<N>> boolean isCut(final Graph<N, E> graph,
                                                       final E edge, final N begin, final N end) {
        if (graph instanceof WeakConnectivityOptimizedGraph) {
            try {
                return ((WeakConnectivityOptimizedGraph) graph).isCut(edge,
                        begin, end);
        ...
        return Topography.isCut(graph, Collections.<N>emptySet(),
                Collections.singleton(edge), begin, end);
    }

    public static <N, E extends Edge<N>> int getNodeConnectivity(
            final Graph<N, E> graph) {
        if (graph instanceof WeakConnectivityOptimizedGraph) {
            try {
                return ((WeakConnectivityOptimizedGraph) graph)
        ...
        final Set<Set<N>> combinations = Counters.everyCombination(graph
                .getNodes());
        final SortedSet<Set<N>> sortedCombinations = new TreeSet<>(
                new SizeComparator());
    ...
    public static <N, E extends Edge<N>> int getEdgeConnectivity(
            final Graph<N, E> graph) {
        if (graph instanceof WeakConnectivityOptimizedGraph) {
            try {
                return ((WeakConnectivityOptimizedGraph) graph)
        ...
        final Set<Set<E>> combinations = Counters.everyCombination(graph
                .getEdges());
        final SortedSet<Set<E>> sortedCombinations = new TreeSet<>(
                new SizeComparator());
        sortedCombinations.addAll(combinations);
    ...
    public static <N, E extends Edge<N>> int getNodeConnectivity(
            final Graph<N, E> graph, final N begin, final N end) {
        if (graph instanceof WeakConnectivityOptimizedGraph) {
            try {
                return ((WeakConnectivityOptimizedGraph) graph)
                        .getNodeConnectivity(begin, end);
            } catch (final UnsupportedOperationException caught) {
                // if it is not supported, lets handle it as if it was not
                // optimized
            }
    ...
    public static <N, E extends Edge<N>> boolean isComplete(
            final Graph<N, E> graph) {
        if (graph instanceof WeakConnectivityOptimizedGraph) {
            try {
    ...
    public static <N, E extends Edge<N>> boolean isSubGraph(
            final Graph<N, E> graph, final Graph<N, E> subgraph) {
    ...
    public static <N, E extends Edge<N>> int getMinimumDegree(
            final Graph<N, E> graph) {
        if (graph instanceof ConnectionismOptimizedGraph) {
            try {
    ...
    public static <N, E extends Edge<N>> boolean isMultigraph(
            final Graph<N, E> graph) {
        if (graph instanceof ConnectionismOptimizedGraph) {
            try {
        ...
        for (final N currentNode : graph.getNodes()) {
            final List<N> neighbors = new ArrayList<>(
                    graph.getAdjacentNodes(currentNode));
    ...
    public static <N, E extends Edge<N>> boolean isIsomorphic(
            final Graph<N, E> graph, final Graph<N, E> isomorphicGraph) {
        if (graph instanceof StructureOptimizedGraph) {
            try {
                return ((StructureOptimizedGraph) graph)
                        .isIsomorphic(isomorphicGraph);
            } catch (final UnsupportedOperationException caught) {
                // if it is not supported, lets handle it as if it was not
                // optimized
            }
        ...
        return (Topography.isHomomorphic(graph, isomorphicGraph) && Topography
                .isHomomorphic(isomorphicGraph, graph));
    }

    public static <N, E extends Edge<N>> boolean isHomomorphic(
            final Graph<N, E> graph, final Graph<N, E> homomorphicGraph) {
        if (graph instanceof StructureOptimizedGraph) {
            try {
                return ((StructureOptimizedGraph) graph)
                        .isHomomorphic(homomorphicGraph);
            } catch (final UnsupportedOperationException caught) {
                // if it is not supported, lets handle it as if it was not
                // optimized
            }
            ...
            for (final N neighborNode : neighborNodes) {
                if (uncheckedNodes.contains(neighborNode)) {
                    if (!homomorphicNodes.contains(neighborNode))
                        return false;
                    if (!homomorphicGraph.getAdjacentNodes(currentNode)
     ...
     * Simple graphs can not have two nodes connected by two edges differing
     ...
     * @since 2.0
     */
    public static <N, E extends Edge<N>> boolean isSimple(
            final Graph<N, E> graph) {
        if (graph instanceof ConnectionismOptimizedGraph) {
            try {
    ...
    /**
     * Determines if every node in this graph has the same degree. If there are
     ...
     * @since 2.0
     */
    public static <N, E extends Edge<N>> boolean isRegular(
            final Graph<N, E> graph) {
        if (graph instanceof ConnectionismOptimizedGraph) {
            try {
     ...
     */
    public static <N, E extends Edge<N>> int getRegularDegree(
            final Graph<N, E> graph) {
        if (graph instanceof ConnectionismOptimizedGraph) {
            try {
     ...
     * @since 2.0
     */
    public static <N, E extends Edge<N>> int getMultiplicity(
            final Graph<N, E> graph) {
        if (graph instanceof ConnectionismOptimizedGraph) {
            try {
        ...
        for (final E edge : graph.getEdges()) {
            final int edgeMultiplicity = Topography
                    .getMultiplicity(graph, edge);
     ...
     * @return the number of edges in the graph with the exact set of end nodes
     * as the specified edge, not including the specified edge itself.
     * @throws IllegalArgumentException if the specified edge is not present in the graph.
     * @see com.syncleus.dann.graph.topological.Topography#getMultiplicity(Graph,
     * com.syncleus.dann.graph.Edge)
     * @since 2.0
     */
    public static <N, E extends Edge<N>> int getMultiplicity(
            final Graph<N, E> graph, final E edge) {
        if (graph instanceof ConnectionismOptimizedGraph) {
            try {
                return ((ConnectionismOptimizedGraph) graph)
                        .getMultiplicity(edge);
            } catch (final UnsupportedOperationException caught) {
                // if it is not supported, lets handle it as if it was not
                // optimized
            }
        ...
        for (final E potentialMultiple : potentialMultiples) {
            if (potentialMultiple.equals(edge))
                continue;
            final List<N> potentialNodes = new ArrayList<>(
                    potentialMultiple.getNodes());
            if (potentialNodes.size() != edgeNodes.size())
                continue;
            edgeNodes.forEach(potentialNodes::remove);
     ...
     * @since 2.0
     */
    public static <N, E extends Edge<N>> boolean isMultiple(
            final Graph<N, E> graph, final E edge) {
        if (graph instanceof ConnectionismOptimizedGraph) {
            try {
     ...
     * the set. if the specified nodes each have at least one traversable edge,
     * all traversable edges go to other nodes in the knot, no traversable edges
     * connect to a node outside of the knot, and there is at least one
     ...
     * @since 2.0
     */
    public static <N, E extends Edge<N>> boolean isKnot(
            final Graph<N, E> graph, final Set<N> knotedNodes) {
    ...
    public static <N, E extends BidirectedEdge<N>> int getIndegree(
            final BidirectedGraph<N, E> graph, final N node) {
        if (graph instanceof ConnectionismOptimizedBidirectedGraph) {
            try {
                return ((ConnectionismOptimizedBidirectedGraph) graph)
                        .getIndegree(node);
            } catch (final UnsupportedOperationException caught) {
                // if it is not supported, lets handle it as if it was not
                // optimized
    ...
    public static <N, E extends BidirectedEdge<N>> int getOutdegree(
            final BidirectedGraph<N, E> graph, final N node) {
        if (graph instanceof ConnectionismOptimizedBidirectedGraph) {
            try {
                return ((ConnectionismOptimizedBidirectedGraph) graph)
    ...
    public static <N, E extends HyperEdge<N>> int getRank(
            final HyperGraph<N, E> graph) {
    ...
    public static <N, E extends HyperEdge<N>> BidirectedGraph<N, BidirectedEdge<N>> getPrimal(
            final HyperGraph<N, E> graph) {
    ...
    public static <N, E extends HyperEdge<N>> boolean isPartial(
            final HyperGraph<N, E> graph, final HyperGraph<N, E> partialGraph) {
        if (graph instanceof StructureOptimizedHyperGraph) {
            try {
                return ((StructureOptimizedHyperGraph) graph)
                        .isPartial(partialGraph);
            } catch (final UnsupportedOperationException caught) {
                // if it is not supported, lets handle it as if it was not
                // optimized
            }
    ...
    public static <N, E extends HyperEdge<N>> boolean isHost(
            final HyperGraph<N, E> graph, final HyperGraph<N, E> hostGraph) {
        if (graph instanceof StructureOptimizedHyperGraph) {
            try {
    ...
    public static <N, E extends HyperEdge<N>> boolean isUniform(
            final HyperGraph<N, E> graph) {
        if (graph instanceof StructureOptimizedHyperGraph) {
            try {
    ...
    /**
     * Removes a set of nodes and a set of edges from the given graph. First, it
     * removes all nodes from the graph. Then, it removes all edges in the edges
     * to remove that are not connected to the given node.
     *
     ...
     */
    private static <N, E extends Edge<N>> ImmutableAdjacencyGraph<N, Edge<N>> deleteFromGraph(
            final Graph<N, E> graph, final Set<N> deleteNodes,
        ...
        final Set<Edge<N>> cutEdges = new HashSet<>(deleteEdges);
        deleteEdges.forEach(cutEdges::remove);
        ...
        final Set<Edge<N>> removeEdges = new HashSet<>();
        final Set<Edge<N>> addEdges = new HashSet<>();
        ...
        removeEdges.forEach(cutEdges::remove);
    ...
    private static class SizeComparator implements Comparator<Collection<?>>,
        ...
        /**
         * Returns true if the given object is a non-null SizeComparator. As
         * this class has no state, all SizeComparators are equal.
         *
        ...
        public int hashCode() {
            ...
            return super.hashCode(); // Default implementation; may need to
            // bring in line to equals
        }
    }
}
