...
public interface WeakConnectivityOptimizedGraph<N, E extends Edge<N>> extends
		Graph<N, E> {
	/**
	 ...
	 *
	 * @param firstNode
	 *            begining node to find a path from.
	 * @param lastNode
	 *            eding node to find a path to.
	 * @return true if a path exists, false otherwise.
	 * @throws IllegalArgumentException
	 *             if either of the specified nodes is not in the graph.
	 * @since 2.0
	 */
	 ...
	 * graph and still be weakly connected. Assuming of course all related edges
	 * are also copied to to the subgraph.
	 *
	...
	/**
	 * Determines if the subgraph is a maximally connected component. A subgraph
	 * is maximally connected if every node in the subgraph has every edge from
	 * the parent graph that is connected to one of the nodes in the subgraph
	 * along with every node that is an end point of one of these edges.
	 *
	 * @param subgraph
	 ...
	 * removing all the specified nodes and edges the number of maximally
	 ...
	 *
	 * @param nodes
	 *            set of nodes to remove when checking for a cut.
	 * @param edges
	 *            set of edges to remove when checking for a cut.
	...
	/**
	 * Determines if the set of edges form a cut. They are a cut if by removing
	 * all the specified edges the number of maximally connected components is
	 * increased.
	 *
	 * @param edges
	 *            set of edges to remove when checking for a cut.
	 ...
	 *
	 * @param node
	 *            node to remove when checking for a cut.
	 ...
	 *
	 * @param edge
	 *            edge to remove when checking for a cut.
	 ...
	 *            set of edges to remove when checking for a cut.
	 * @param begin
	 *            begining node in the path to check for a cut.
	 * @param end
	 *            ending node in the path to check for a cut.
	 * @return true if the nodes and edges form a cut between begin and end,
	 *         false otherwise.
	 * @throws IllegalArgumentException
	 *             if any of nodes, edges, begin, or end are not in this graph
	 *             or begin and end have no path between them before the cut.
	 * @since 2.0
	 */
	boolean isCut(Set<N> nodes, Set<E> edges, N begin, N end);

	/**
	 * Determines if the set of edges form a cut between begin and end nodes.
	 * They are a cut if by removing all the specified edges there is no longer
	 * a path between begin and end.
	 *
	 * @param edges
	 *            set of edges to remove when checking for a cut.
	 * @param begin
	 ...
	 * edges. Its important to note that when removing a node that node will
	 * also be removed as an end point from all edges. If as a result a edge is
	 * left with a single end point then that edge will also be removed.
	 *
	 ...
	 *
	 * @param begin
	 *            begining node in the path to check for connectivity.
	 * @param end
	 *            ending node in the path to check for connectivity.
	 * @return The minimum number of nodes that must be removed from the graph
	 *         in order to form a cut between the specified nodes.
	 * @throws IllegalArgumentException
	 *             if either begin or end is not in the graph.
	 * @since 2.0
	 */
