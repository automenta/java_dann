...
public interface StructureOptimizedGraph<N, E extends Edge<N>> extends
		Graph<N, E> {
	/**
	 ...
	 * return true.
	 *
	 * @return true if the specified graph only contains nodes and edges that
	 *         are also present in this graph.
	 * @since 2.0
	 */
	 ...
	 * graph are adjacent in the specified graph, and vice versa. For bidirected
	 * edges this means both graphs have equal nodes and the same number of
	 * edges each eith the same end points however the traversability of the
	 * edges may differ between graphs. In Hypergraphs there can be an entirely
	 * different number of edges and still be isomorphic. If there are no edges
	 * and/or there are no nodes in both graphs then they are isomorphic.
	 *
	 * @param isomorphicGraph
	 *            graph to check if it is isomorphic with this graph.
	 * @return true if this graph and the specified graph have all the same
	 *         nodes and all the same pairs of adjacecy betwee them. False
	 *         otherwise.
	 * @since 2.0
	 */
	 ...
	 * homomorphic.
	 *
	 * @param homomorphicGraph
	 *            graph to check if it is homomorphic with this graph.
