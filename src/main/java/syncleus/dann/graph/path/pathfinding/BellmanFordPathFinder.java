...
public class BellmanFordPathFinder<N, E extends DirectedEdge<N>> implements
        PathFinder<N, E> {
    ...
    public void calculateSteps(final N begin) {
        ...
        for (final E edge : edges) {
            if (edge.getDestinationNode() == begin)
                continue;
            final PathedStep sourcePathedStep = pathedSteps.get(edge
                    .getSourceNode());
            final PathedStep destinationPathedStep = pathedSteps.get(edge
                    .getDestinationNode());
            ...
            if (destinationPathedStep.updateParent(sourcePathedStep, edge))
                throw new NegativeWeightCycleException(
                        "negative-weight cycle found in graph");
        }
    }
