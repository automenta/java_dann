 ...
 * @since 2.0
 */
public abstract class AbstractTreeAdjacencyGraph<N, E extends BidirectedEdge<N>>
        extends AbstractBidirectedAdjacencyGraph<N, E> implements
        TreeGraph<N, E>, TreeOptimizedGraph<N, E> {
    // TODO restrict tree's to only maximally connected trees or perhaps just
    // tree in general
    ...
    public boolean isSpanningTree(final Graph<N, E> subGraph) {
        throw new UnsupportedOperationException(
                "We have not optimized for this, allow default algorithms to calcualte");
    }
    ...
    public boolean isLeaf(final E edge) {
        return edge.getNodes().stream().anyMatch((node) -> (this.isLeaf(node)));
    }
    ...
    @Override
    public AbstractTreeAdjacencyGraph<N, E> cloneAdd(final Set<N> newNodes,
                                                     final Set<E> newEdges) {
        return (AbstractTreeAdjacencyGraph<N, E>) super.cloneAdd(newNodes,
                newEdges);
    }

    @Override
    public AbstractTreeAdjacencyGraph<N, E> cloneRemove(final E edgeToRemove) {
        return (AbstractTreeAdjacencyGraph<N, E>) super
    ...
    @Override
    public AbstractTreeAdjacencyGraph<N, E> cloneRemove(
            final Set<N> deleteNodes, final Set<E> deleteEdges) {
        return (AbstractTreeAdjacencyGraph<N, E>) super.cloneRemove(
