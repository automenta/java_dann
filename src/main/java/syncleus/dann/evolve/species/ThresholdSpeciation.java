...
public abstract class ThresholdSpeciation implements Speciation, Serializable {
    ...
    private void divideByFittestSpecies(final List<Species> speciesCollection,
        ...
        final Species bestSpecies = findBestSpecies();
     ...
     */
    public void removeSpecies(final Species species) {
    ...
    private void levelOff() {
        ...
        total = list.stream().map(Species::getOffspringCount).reduce(total, Integer::sum);
     ...
     */
    private List<Genome> resetSpecies(final List<Genome> inputGenomes) {
        final List<Genome> result = new ArrayList<>();
        final Object[] speciesArray = this.population.getSpecies().toArray();

        inputGenomes.stream().forEach(result::add);
