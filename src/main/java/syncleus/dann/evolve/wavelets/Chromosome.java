...
/**
 * A Chromosome is a mutable collection of Chromatids for use in a genetic
 * algorithm.
 */
public class Chromosome implements Cloneable {
	 ...
	 *
	 * @param external
	 *            Whether the caller is external to the Chromosome
	...
	Set<SignalKey> getExpressedSignals(final boolean external) {
		final Set<SignalKey> allSignals = new HashSet<SignalKey>(
				this.leftChromatid.getExpressedSignals(external));
	 ...
	 *
	 * @param concentration
	 *            The signal key concentration
	 * @param isExternal
	 *            Whether this signal is external to the Chromosome
	 * @return Whether the bind was successful
	 * @see com.syncleus.dann.genetics.wavelets.WaveletChromatid#bind(SignalKeyConcentration,
	 *      boolean)
	 */
	...
	public List<AbstractWaveletGene> getGenes() {
		final List<AbstractWaveletGene> genes = new ArrayList<AbstractWaveletGene>(
				this.leftChromatid.getGenes());
	...
	public List<PromoterGene> getPromoterGenes() {
		final List<PromoterGene> promoters = new ArrayList<PromoterGene>(
				this.leftChromatid.getPromoterGenes());
	...
	public List<SignalGene> getLocalSignalGenes() {
		final List<SignalGene> localSignalGenes = new ArrayList<SignalGene>(
				this.leftChromatid.getLocalSignalGenes());
	...
	public List<ExternalSignalGene> getExternalSignalGenes() {
		final List<ExternalSignalGene> externalSignalGenes = new ArrayList<ExternalSignalGene>(
				this.leftChromatid.getExternalSignalGenes());
		externalSignalGenes
				.addAll(this.rightChromatid.getExternalSignalGenes());
	 ...
	 *
	 * @param deviation
	 *            The amount of deviation allowable
	 */
	private void crossover(final double deviation) {
		// find the crossover position
		final int crossoverPosition;
		if (RANDOM.nextBoolean()) {
			final int length = (this.leftChromatid.getCentromerePosition() < this.rightChromatid
					.getCentromerePosition() ? this.leftChromatid
					.getCentromerePosition() : this.rightChromatid
					.getCentromerePosition());

			final int fromEnd = (int) Math.abs((new MutableDouble(0d)).mutate(
			...
			final int leftLength = this.leftChromatid.getGenes().size()
					- this.leftChromatid.getCentromerePosition();
			final int rightLength = this.rightChromatid.getGenes().size()
					- this.leftChromatid.getCentromerePosition();

			final int length = (leftLength < rightLength ? leftLength
					: rightLength);
		...
		final List<AbstractWaveletGene> leftGenes = this.leftChromatid
				.crossover(crossoverPosition);
		final List<AbstractWaveletGene> rightGenes = this.rightChromatid
				.crossover(crossoverPosition);
