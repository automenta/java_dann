...
public class ExpressionFunction implements Cloneable {
    ...
    /**
     * Creates the ExpressionFunction as a copy of the supplied
     * ExpressionFunction.
     *
    ...
    public ExpressionFunction(final ExpressionFunction copy) {
        ...
        this.waves = new ArrayList<>(copy.waves);
    ...
    public boolean receives(final SignalKey signal) {
        return this.receptors.stream().anyMatch((receptor) -> (receptor.binds(signal)));
    ...
    public double calculate(
        ...
        this.receptors.stream().forEach((receptor) -> {
            ...
            concentration = signalConcentrations.stream().filter((signalConcentration) -> (receptor.binds(signalConcentration.getSignal()))).map(SignalKeyConcentration::getConcentration).reduce(concentration, (accumulator, _item) -> accumulator + _item);
    ...
    public ExpressionFunction clone() {
        try {
            ...
            copy.receptors = new HashSet<>(this.receptors);
            final List<WaveMultidimensionalFunction> newWaves = new ArrayList<>();
            this.waves.stream().forEach((wave) -> newWaves.add(wave.clone()));
            copy.waves = newWaves;
    ...
    private void reconstructWavelet() {
        ...
        this.waves.stream().forEach(this.wavelet::addWave);
    }
    ...
    public ExpressionFunction mutate(final double deviation) {
        ...
        while (RANDOM.nextFloat() < changePercentage) {
            ...
            if ((this.waves.size() > 1)
                    && (RANDOM.nextDouble() < changePercentage)) {
            ...
            if ((this.receptors.size() > 1)
                    && (RANDOM.nextDouble() < changePercentage)) {
                final ReceptorKey[] receptorArray = new ReceptorKey[copy.receptors
                        .size()];
                copy.receptors.toArray(receptorArray);
                final ReceptorKey deleteReceptor = receptorArray[RANDOM
                        .nextInt(receptorArray.length)];
                copy.receptors.remove(deleteReceptor);
                final ReceptorKey[] copyReceptors = new ReceptorKey[copy.receptors
                        .size()];
    ...
    private WaveMultidimensionalFunction generateNewWave() {
        ...
        if (newWave.getForm() <= 0.0)
            newWave.setForm(newWave.getForm()
                    + ((1 + RANDOM.nextGaussian()) * PHASE_ADJUSTMENT));
        ...
        if (!this.waves.isEmpty()) {
            ...
            if (RANDOM.nextDouble() <= changePercentage)
                newWave.setFrequency(newWave.getFrequency()
                        + ((RANDOM.nextFloat() * 2 - 1) * frequencyAdjustment));
            if (RANDOM.nextDouble() <= changePercentage)
                newWave.setPhase(newWave.getPhase()
                        + ((RANDOM.nextFloat() * 2 - 1) * PHASE_ADJUSTMENT));
            if (RANDOM.nextDouble() <= changePercentage)
                newWave.setAmplitude(newWave.getAmplitude()
                        + ((RANDOM.nextFloat() * 2 - 1) * PHASE_ADJUSTMENT));
            if (RANDOM.nextDouble() <= changePercentage)
                newWave.setForm(newWave.getForm()
                        + (RANDOM.nextFloat() * frequencyAdjustment));
            if (RANDOM.nextDouble() <= changePercentage)
