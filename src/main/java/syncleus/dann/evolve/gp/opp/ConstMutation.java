...
public class ConstMutation implements EvolutionaryOperator {
     ...
     * @param theFrequency The frequency of mutation.
     * @param theSigma     The sigma to use for mutation.
     */
    ...
    private void mutateNode(final Random rnd, final ProgramNode node) {
        ...
        node.getChildNodes().stream().map((n) -> (ProgramNode) n).forEach((childNode) -> mutateNode(rnd, childNode));
    }
