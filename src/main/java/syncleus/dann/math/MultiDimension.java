...
public class MultiDimension {
     ...
     */
    public MultiDimension(final int n) {
        this.dimensions = new int[n];
    }

    public MultiDimension(final MultiDimension lower) {
     ...
     */
    public int getDimension(final int d) {
     ...
     *
     * @param constraint The dimension constraints.
     * @return True if there are still more combinations, false if we are done.
     */
    public boolean forward(final DimensionConstraint constraint) {
        int i = 0;
        do {
            this.dimensions[i]++;
     ...
     */
    public int flatten(final DimensionConstraint constraint) {
        int result = 0;
        int mult = 1;
     ...
     */
    public void setDimension(final int d, final int value) {
    ...
    public double calculateLowerStep(final DimensionConstraint constraint,
                                     final int d) {
    ...
    public double calculateUpperStep(final DimensionConstraint constraint,
                                     final int d) {
        if (this.dimensions[d] <= constraint.getLower(d))
            return 0;
        else
