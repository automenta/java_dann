...
public class SchurEigenvalueDecomposition implements java.io.Serializable,
	...
	public SchurEigenvalueDecomposition(final RealMatrix matrixToDecompose) {
		...
		this.realEigenvalues.addAll(Collections.nCopies(width, new RealNumber(
				0.0)));
		this.imaginaryEigenvalues = new ArrayList<RealNumber>(width);
		this.imaginaryEigenvalues.addAll(Collections.nCopies(width,
				new RealNumber(0.0)));
		...
		final HessenbergDecomposition hessenberbDecomposition = new NonsymetricHessenbergReduction(
	...
	private void schurReduction() {
		...
		while (n >= low) {
			...
			int l = n;
			while (l > low) {
				s = Math.abs(hessenberg[l - 1][l - 1])
						+ Math.abs(hessenberg[l][l]);
				if (s == 0.0)
					s = norm;
			...
			if (l == n) {
				...
				if (iter == 10) {
					...
					for (int i = low; i <= n; i++)
						hessenberg[i][i] -= x;
					s = Math.abs(hessenberg[n][n - 1])
							+ Math.abs(hessenberg[n - 1][n - 2]);
					x = 0.75 * s;
					y = x;
					w = -0.4375 * s * s;
				}
				...
				while (m >= l) {
					...
					if (m == l)
						break;
					if (Math.abs(hessenberg[m][m - 1])
							* (Math.abs(q) + Math.abs(r)) < eps
							* (Math.abs(p) * (Math
									.abs(hessenberg[m - 1][m - 1])
									+ Math.abs(z) + Math
										.abs(hessenberg[m + 1][m + 1]))))
						break;
					m--;
				...
				for (int k = m; k <= n - 1; k++) {
					...
					if (s != 0) {
						...
						for (int j = k; j < nn; j++) {
							...
							if (notlast) {
								p += r * hessenberg[k + 2][j];
								hessenberg[k + 2][j] -= p
                                                                        * z;
							}
							hessenberg[k][j] -= p * x;
							hessenberg[k + 1][j] -= p * y;
						}
						...
						for (int i = 0; i <= Math.min(n, k + 3); i++) {
							...
							if (notlast) {
								p += z * hessenberg[i][k + 2];
								hessenberg[i][k + 2] -= p
                                                                        * r;
							}
							hessenberg[i][k] -= p;
							hessenberg[i][k + 1] -= p * q;
						}

						// Accumulate transformations
						for (int i = low; i <= high; i++) {
							p = x * eigenVectors[i][k] + y
									* eigenVectors[i][k + 1];
							if (notlast) {
		...
		for (n = nn - 1; n >= 0; n--) {
			...
			if (q == 0) {
				...
				for (int i = n - 1; i >= 0; i--) {
					...
					r = 0.0;
					for (int j = l; j <= n; j++)
						r += hessenberg[i][j] * hessenberg[j][n];
					if (e[i] < 0.0) {
						z = w;
						s = r;
					} else {
				...
				if (Math.abs(hessenberg[n][n - 1]) > Math
						.abs(hessenberg[n - 1][n])) {
					hessenberg[n - 1][n - 1] = q / hessenberg[n][n - 1];
					hessenberg[n - 1][n] = -(hessenberg[n][n] - p)
							/ hessenberg[n][n - 1];
				} else {
					cdiv(0.0, -hessenberg[n - 1][n], hessenberg[n - 1][n - 1]
							- p, q);
				...
				for (int i = n - 2; i >= 0; i--) {
					...
					sa = 0.0;
					for (int j = l; j <= n; j++) {
						ra += hessenberg[i][j] * hessenberg[j][n - 1];
						sa += hessenberg[i][j] * hessenberg[j][n];
					}
					w = hessenberg[i][i] - p;
					...
					if (e[i] < 0.0) {
						...
						if (e[i] == 0) {
							...
							if (Math.abs(x) > (Math.abs(z) + Math.abs(q))) {
								hessenberg[i + 1][n - 1] = (-ra - w
										* hessenberg[i][n - 1] + q
										* hessenberg[i][n])
										/ x;
								hessenberg[i + 1][n] = (-sa - w
										* hessenberg[i][n] - q
										* hessenberg[i][n - 1])
										/ x;
							} else {
								cdiv(-r - y * hessenberg[i][n - 1], -s - y
										* hessenberg[i][n], z, q);
						...
						// Overflow control
						t = Math.max(Math.abs(hessenberg[i][n - 1]),
								Math.abs(hessenberg[i][n]));
						if ((eps * t) * t > 1)
							for (int j = i; j <= n; j++) {
								hessenberg[j][n - 1] /= t;
								hessenberg[j][n] /= t;
		...
		for (int j = nn - 1; j >= low; j--)
			for (int i = low; i <= high; i++) {
				...
				for (int k = low; k <= Math.min(j, high); k++)
					z += eigenVectors[i][k] * hessenberg[k][j];
				eigenVectors[i][j] = z;
			}
