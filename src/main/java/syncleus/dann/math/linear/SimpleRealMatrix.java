...
import syncleus.dann.math.linear.decomposition.DoolittleLuDecomposition;
import syncleus.dann.math.linear.decomposition.HouseholderQrDecomposition;
import syncleus.dann.math.linear.decomposition.LuDecomposition2;
import syncleus.dann.math.linear.decomposition.QrDecomposition2;
import syncleus.dann.math.linear.decomposition.StewartSingularValueDecomposition;
...
public class SimpleRealMatrix implements Cloneable, Serializable, RealMatrix {
		...
		for (int heightIndex = 0; heightIndex < this.height; heightIndex++) {
			if (matrixElements[heightIndex].length != this.width)
				throw new IllegalArgumentException(
						"All rows must have the same length.");
			this.matrixElements[heightIndex] = Arrays.copyOf(
					matrixElements[heightIndex], this.width);
		}
	}
	 ...
	 *
	 * @param packedMatrixElements
	 *            One-dimensional array of doubles, packed by columns (ala
	 *            Fortran).
	 * @param height
	 *            Number of rows.
	 * @throws IllegalArgumentException
	 *             Array length must be a multiple of height.
	 */
	public SimpleRealMatrix(final double[] packedMatrixElements,
			final int height) {
		this.height = height;
		this.width = ((height == 0) ? 0
				: (packedMatrixElements.length / height));
	...
	public SimpleRealMatrix clone() {
		try {
			...
			for (int heightIndex = 0; heightIndex < this.height; heightIndex++)
				copy.matrixElements[heightIndex] = this.matrixElements[heightIndex]
						.clone();
			return copy;
	...
	public double[] getRowPackedCopy() {
		...
		for (int i = 0; i < this.height; i++)
			System.arraycopy(this.matrixElements[i], 0, values, i * this.width,
					this.width);
		return values;
	}
	 ...
	 *            Final column index
	 * @param fillMatrix
	 *            the source matrix to use to fill the specified elements of
	 *            this matrix.
	...
	public void setMatrix(final int heightStart, final int heightEnd,
			final int widthStart, final int widthEnd,
			final SimpleRealMatrix fillMatrix) {
	 ...
	 * @param fillMatrix
	 *            source matrix used to fill the specified elements.
	 * @throws ArrayIndexOutOfBoundsException
	 *             Sub-matrix indices
	 */
	...
	public void setMatrix(final int[] heightIndexes, final int widthStart,
			final int widthEnd, final SimpleRealMatrix fillMatrix) {
		for (int i = 0; i < heightIndexes.length; i++)
			for (int j = widthStart; j <= widthEnd; j++)
	 ...
	 */
	public void setMatrix(final int heightStart, final int heightEnd,
			final int[] widthIndexes, final SimpleRealMatrix fillMatrix) {
		for (int i = heightStart; i <= heightEnd; i++)
			for (int j = 0; j < widthIndexes.length; j++)
	...
	public RealMatrix add(final RealNumber operand) {
		final SimpleRealMatrix newSimpleMatrix = new SimpleRealMatrix(
				this.height, this.width);
		final double[][] newMatrix = newSimpleMatrix.matrixElements;
		for (int heightIndex = 0; heightIndex < this.height; heightIndex++)
			for (int widthIndex = 0; widthIndex < this.width; widthIndex++)
				newMatrix[heightIndex][widthIndex] = this.matrixElements[heightIndex][widthIndex]
						+ operand.getValue();
	...
	public RealMatrix add(final double scalar) {
		final SimpleRealMatrix newMatrix = new SimpleRealMatrix(this.height,
				this.width);
	...
	public RealMatrix addEquals(final RealMatrix operand) {
		...
		for (int heightIndex = 0; heightIndex < this.height; heightIndex++)
			for (int widthIndex = 0; widthIndex < this.width; widthIndex++)
				newMatrixElements[heightIndex][widthIndex] = newMatrixElements[heightIndex][widthIndex]
						+ operand.get(heightIndex, widthIndex);
	...
	public RealMatrix arrayRightDivide(final RealMatrix operand) {
		checkMatrixDimensions(operand);
		final SimpleRealMatrix matrix = new SimpleRealMatrix(this.height,
				this.width);
	...
	public RealMatrix multiply(final RealMatrix operand) {
		...
		final SimpleRealMatrix resultMatrix = new SimpleRealMatrix(this.height,
				operand.getWidth());
	...
	public RealMatrix solve(final RealMatrix operand) {
		return (this.height == this.width ? (new DoolittleLuDecomposition<RealMatrix, RealNumber>(
				this)).solve(operand)
				: (new HouseholderQrDecomposition<RealMatrix, RealNumber>(this))
						.solve(operand));
	}
	...
	public double getDeterminant() {
		return new DoolittleLuDecomposition<RealMatrix, RealNumber>(this)
	...
	private void checkMatrixDimensions(final RealMatrix compareMatrix) {
		if (compareMatrix.getHeight() != this.height
				|| compareMatrix.getWidth() != this.width)
	 ...
	 *            The matrix to add.
	 */
	public void add(final SimpleRealMatrix theMatrix) {
	...
	public boolean equals(final SimpleRealMatrix theMatrix, final int precision) {
		...
		for (int r = 0; r < getRows(); r++) {
			for (int c = 0; c < getCols(); c++) {
				if ((long) (this.matrixElements[r][c] * actualPrecision) != (long) (data[r][c] * actualPrecision)) {
					return false;
				}
			}
		}
	...
	public double[][] getArrayCopy() {
		...
		for (int i = 0; i < getRows(); i++) {
			for (int j = 0; j < getCols(); j++) {
				result[i][j] = this.matrixElements[i][j];
			}
		}
		return result;
	}
	 ...
	 */
	public SimpleRealMatrix getCol(final int col) {
		if (col > getCols()) {
	 ...
	 */
	public int getCols() {
		return this.matrixElements[0].length;
	}
	...
	public SimpleRealMatrix getMatrix(final int i0, final int i1, final int j0, final int j1) {
		...
		try {
			for (int i = i0; i <= i1; i++) {
				for (int j = j0; j <= j1; j++) {
					b[i - i0][j - j0] = this.matrixElements[i][j];
				}
			}
	 ...
	 * @return The specified submatrix.
	 */
	public SimpleRealMatrix getMatrix(final int i0, final int i1, final int[] c) {
	...
	public SimpleRealMatrix getMatrix(final int[] r, final int j0, final int j1) {
		...
		try {
			for (int i = 0; i < r.length; i++) {
				for (int j = j0; j <= j1; j++) {
					b[i][j - j0] = this.matrixElements[r[i]][j];
				}
			}
		} catch (final ArrayIndexOutOfBoundsException e) {
	 ...
	 * @return The specified submatrix.
	 */
	public SimpleRealMatrix getMatrix(final int[] r, final int[] c) {
	 ...
	 * @return A matrix.
	 */
	public SimpleRealMatrix getRow(final int row) {
		if (row > getRows()) {
		...
		for (int col = 0; col < getCols(); col++) {
			newMatrix[0][col] = this.matrixElements[row][col];
		}
