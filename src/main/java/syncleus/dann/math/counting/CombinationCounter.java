    ...
    public CombinationCounter(final int setSize, final int combinationSize) {
        if (combinationSize > setSize)
            throw new IllegalArgumentException(
                    "combinationSize can not be larger than setSize");
        if (setSize < 0)
            throw new IllegalArgumentException("setSize can not be negative");
        if (combinationSize < 0)
            throw new IllegalArgumentException(
                    "combinationSize can not be negative");
        ...
        final BigInteger nminusrFact = calculateFactorial(setSize
                - combinationSize);
        this.total = (setSize == 0 || combinationSize == 0 ? BigInteger.ZERO
