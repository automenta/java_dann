...
public final class EngineArray {
    ...
    private static void arrayCopy(final byte[] src, final byte[] dst) {
    ...
    public static int indexOfLargest(final double[] data) {
        int result = -1;
    ...
    public static double min(final double[] weights) {
    ...
    public static double max(final double[] weights) {
    ...
    public static int maxIndex(final double[] data) {
        int result = -1;
        for (int i = 0; i < data.length; i++) {
            if (result == -1 || data[i] > data[result]) {
                result = i;
    ...
    public static int maxIndex(final int[] data) {
        int result = -1;
        for (int i = 0; i < data.length; i++) {
            if (result == -1 || data[i] > data[result]) {
                result = i;
    ...
    public static double mean(final int[] data) {
        double result = 0;
        for (int i = 0; i < data.length; i++) {
            result += data[i];
        }
    ...
    public static double sdev(final int[] data) {
        final double avg = mean(data);
        double result = 0;
        for (int i = 0; i < data.length; i++) {
            final double diff = data[i] - avg;
            result += diff * diff;
        }
        return Math.sqrt(result / data.length);
    }

    public static double euclideanDistance(final double[] p1, final double[] p2) {
        double sum = 0;
    ...
    public static void fill(final double[][] sigma, final int value) {
    ...
    public static void fill(final boolean[] a, final boolean b) {
        for (int i = 0; i < a.length; i++) {
            a[i] = b;
        }
    }

    public static double[] add(final double[] d, final double[] m) {
    ...
    public static void fill(final int[] a, final int value) {
        Arrays.fill(a, value);
    }

    public static String replace(final String str, final String searchFor,
        ...
        for (int i = 0; i < str.length(); i++) {
            ...
            if (i + searchFor.length() < str.length()) {
                final String cmp = str.substring(i, i + searchFor.length());
    ...
    public static void arrayCopy(final int[] source, final int sourcePos,
                                 final int[] target, final int targetPos, final int length) {
        System.arraycopy(source, sourcePos, target, targetPos, length);

    }

    public static void arrayAdd(final double[][] target, final double[][] h) {
