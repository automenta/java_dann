 ...
 */
package syncleus.dann.learn.train.strategy;
...
public class RequiredImprovementStrategy implements Strategy {
     ...
     */
    private final double acceptableThreshold;
     ...
     *
     * @param required  The required error rate.
     * @param threshold The accepted threshold, don't reset if error is below this.
     * @param cycles    The number of cycles to reach that rate.
     */
    public RequiredImprovementStrategy(final double required,
                                       final double threshold, final int cycles) {
    ...
    public void preIteration() {
        ...
        if (train.getError() > this.acceptableThreshold) {
            if (!Double.isNaN(lastError)) {
                final double improve = (lastError - train.getError());
