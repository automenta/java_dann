...
public class SimpleDynamicGraphicalModelNode<S> extends
    ...
    public SimpleDynamicGraphicalModelNode(final int historyCapacity,
                                           final S initialState) {
        super(initialState);

        if (historyCapacity < 0)
            throw new IllegalArgumentException(
                    "historyCapacity can not be less than 0");
        if (initialState == null)
            throw new IllegalArgumentException("initialState can not be null");

        final List<SimpleGraphicalModelNode<S>> newHistoricalNodes = new ArrayList<>(
                historyCapacity);
    ...
    public SimpleDynamicGraphicalModelNode(final List<S> history,
                                           final S initialState) {
        super(initialState);
        if (history == null)
        ...
        final List<SimpleGraphicalModelNode<S>> newHistoricalNodes = new ArrayList<>(
                history.size());
        history.stream().forEach((aHistory) -> newHistoricalNodes.add(new SimpleGraphicalModelNode<>(aHistory)));
        this.historicalNodes = Collections.unmodifiableList(newHistoricalNodes);
    }
            ...
            for (final SimpleGraphicalModelNode<S> historicalNode : this.historicalNodes)
                if (!historicalNode.joiningGraph(graph))
                    throw new IllegalStateException(
                            "historical node will not attach to graph when its parent will");
            return true;
        } else
            return false;
    }
        ...
        if (super.leavingGraph(graph)) {
            ...
            for (final SimpleGraphicalModelNode<S> historicalNode : this.historicalNodes)
                if (!historicalNode.leavingGraph(graph))
                    throw new IllegalStateException(
                            "historical node will not detach from graph when its parent will");
            return true;
        } else
            return false;
    }
    ...
    public List<S> getStateHistory() {
        final List<S> historyStates = new ArrayList<>(
                this.getStateHistoryCapacity());
        this.historicalNodes.stream().forEach((node) -> historyStates.add(node.getState()));
    ...
    public void setStateHistory(final List<S> history) {
        for (int historyIndex = 0; historyIndex < this.historicalNodes.size(); historyIndex++)
            this.historicalNodes.get(historyIndex).setState(
                    (history.size() > historyIndex ? history.get(historyIndex)
                            : null));
    }
    ...
    public void learnState(final boolean updateHistory) {
        ...
        if (updateHistory) {
            ...
            for (int historyIndex = 0; historyIndex < (this.historicalNodes
                    .size() - 1); historyIndex++)
                this.historicalNodes.get(historyIndex + 1).setState(
                        this.historicalNodes.get(historyIndex).getState());
