...
public class BayesianTable implements Serializable {
    ...
    public BayesianTable(final BayesianEvent theEvent) {
        this.event = theEvent;
        reset();
    }
    ...
    public void reset() {
        this.lines.clear();
        final List<BayesianEvent> parents = this.event.getParents();
        final int l = parents.size();

        final int[] args = new int[l];
     ...
     */
    public void addLine(final double prob, final boolean result,
                        final boolean... args) {
     ...
     * @param args   The arguments.
     */
    public void addLine(final double prob, final int result,
                        final boolean... args) {
        final int[] d = new int[args.length];
        for (int i = 0; i < args.length; i++) {
            d[i] = args[i] ? 0 : 1;
     ...
     * @param args   The arguments.
     */
    public void addLine(final double prob, final int result, final int... args) {
    ...
    public void validate() {
        if (this.lines.size() != this.getMaxLines()) {
            throw new BayesianError("Truth table for " + this.event.toString()
                    + " only has " + this.lines + " line(s), should have "
                    + this.getMaxLines() + " line(s).");
        }
     ...
     * @return The result.
     */
    public int generateRandom(final int... args) {
     ...
     */
    public TableLine findLine(final int result, final int[] args) {

        for (final TableLine line : this.lines) {
            if (line != null && line.compareArgs(args)) {
                if (Math.abs(line.getResult() - result) < EncogMath.DEFAULT_EPSILON) {
                    return line;
                }
            }
        }
