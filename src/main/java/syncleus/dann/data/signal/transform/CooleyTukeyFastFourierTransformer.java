 ...
 * reduce computation time for O(N log N) for smooth numbers. See <a
 * href="http://en.wikipedia.org/wiki/Cooley-Tukey_FFT_algorithm">the wikipedia
 * page</a> for more information.
 *
 * @author Jeffrey Phillips Freeman
 */
public class CooleyTukeyFastFourierTransformer implements
	 ...
	 *            The bit-rate to use.
	 */
	public CooleyTukeyFastFourierTransformer(final int ourBlockSize,
			final int ourBitRate) {
	...
	public double[] inverseTransform(final DiscreteFourierTransform transform) {
		final ComplexNumber[] complexSignal = inverseTransformMatrix(pad(transform
				.getTransform()));
	 ...
	 * {@inheritDoc}
	 *
	 * @see #circularConvolveMatrix(com.syncleus.dann.math.ComplexNumber[],
	 *      com.syncleus.dann.math.ComplexNumber[])
	 */
	@Override
	public double[] circularConvolve(final double[] first, final double[] second) {
		...
		final ComplexNumber[] resultComplex = circularConvolveMatrix(
	...
	public double[] linearConvolve(final double[] first, final double[] second) {
		...
		final ComplexNumber[] resultComplex = linearConvolveMatrix(
				firstComplex, secondComplex);
		return complexArrayToDoubleArray(resultComplex);
	}
	...
	/**
	 * {@inheritDoc} If the block size is a not a power of 2, the block size is
	 * set to the next-largest power of two.
	 */
	@Override
	 ...
	 *
	 * @param from
	 *            The ComplexNumber to convert from
	...
	/**
	 * Returns a double[] array containing the same elements as signal, with
	 * length equal to the current block size.
	 *
	 * @param signal
	 *            The signal to copy
	...
	private ComplexNumber[] pad(final ComplexNumber[] signal) {
		if (signal.length < this.blockSize) {
			final ComplexNumber[] paddedSignal = Arrays.copyOf(signal,
					this.blockSize);
	 ...
	 *
	 * @param dataPoints
	 *            The data points to transform
	 * @return The transformed matrix
	 */
	public static ComplexNumber[] transformMatrix(
			final ComplexNumber[] dataPoints) {
		final int dataPointCount = dataPoints.length;
		if (!isPowerOf2(dataPointCount))
			throw new IllegalArgumentException(
					"dataPoints size is not a power of 2");
		if (dataPointCount == 1)
		...
		for (int transformIndex = 0; transformIndex < (dataPointCount / 2); transformIndex++) {
			...
			final double kth = correction * Math.PI * transformIndex
					/ dataPointCount;
			final ComplexNumber wk = new ComplexNumber(Math.cos(kth),
					Math.sin(kth));
			completeTransform[transformIndex] = evenTransform[transformIndex]
					.add(wk.multiply(oddTransform[transformIndex]));
			completeTransform[transformIndex + (dataPointCount / 2)] = evenTransform[transformIndex]
					.subtract(wk.multiply(oddTransform[transformIndex]));
		}
		return completeTransform;
	}

	public static ComplexNumber[] inverseTransformMatrix(
			final ComplexNumber[] transforms) {
		...
		for (int signalIndex = 0; signalIndex < transformSize; signalIndex++)
			signal[signalIndex] = signal[signalIndex]
					.multiply(1.0 / (transformSize));
		return signal;
	}

	public static ComplexNumber[] circularConvolveMatrix(
			final ComplexNumber[] first, final ComplexNumber[] second) {
		if (first.length != second.length)
			throw new IllegalArgumentException(
					"first and second must have the same number of elements");
		...
		for (int dataPointIndex = 0; dataPointIndex < dataPointsSize; dataPointIndex++)
			result[dataPointIndex] = firstTransformed[dataPointIndex]
					.multiply(secondTransformed[dataPointIndex]);
	 ...
	 * @see com.syncleus.dann.dataprocessing.signal.transform.CooleyTukeyFastFourierTransformer#circularConvolveMatrix(com.syncleus.dann.math.ComplexNumber[],
	 *      com.syncleus.dann.math.ComplexNumber[])
	 */
	public static ComplexNumber[] linearConvolveMatrix(
			final ComplexNumber[] first, final ComplexNumber[] second) {
		final ComplexNumber[] firstLinear = Arrays.copyOf(first,
				first.length * 2);
		...
		final ComplexNumber[] secondLinear = Arrays.copyOf(second,
				second.length * 2);
