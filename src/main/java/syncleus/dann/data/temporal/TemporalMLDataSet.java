...
public class TemporalMLDataSet extends BasicNeuralDataSet implements
     ...
     * Construct a dataset.
     *
     * @param inputWindowSize   What is the input window size.
     * @param predictWindowSize What is the prediction window size.
     */
    ...
    public void calculateNeuronCounts() {
        ...
        this.descriptions.stream().map((desc) -> {
            ...
            return desc;
        }).filter((desc) -> (desc.isPredict())).forEach((_item) -> this.outputNeuronCount += this.predictWindowSize);
    }
     ...
     *
     * @param when The time that this point should be created at.
