 ...
 * href="http://tartarus.org/~martin/PorterStemmer/"> Martin Porter's stemming
 * algorithm</a>. This algorithm efficiently separates word stems from words.
 *
...
public class PorterStemmer implements Stemmer {
	 ...
	 * exclusion. A, e, i, o, u are not consonants. y is a consonant when it is
	 * at the start of the stem, or when it immediately follows a vowel. All
	 * other characters are consonants.
	 *
	 * @param index
	 *            The index to check
	...
	/**
	 * Returns the number of consonants in the stem via a complex linear count.
	 *
	 ...
	 * succession, once at the given index and once before. 'tall' has the
	 * repeated consonant 'l' at index 3. The theoretical word stem 'lawl' does
	 * not have such a repeated consonant.
	 *
	 * @param testCharacterIndex
	...
	private boolean isRepeatedConsonant(final int testCharacterIndex) {
		return testCharacterIndex >= this.stemStartIndex + 1
				&& this.buffer[testCharacterIndex] == this.buffer[testCharacterIndex - 1]
				&& this.isConsonant(testCharacterIndex);
	}

	/**
	 * Determines whether the given index is the third character in a
	 * consonant-vowel-consonant sequence. 'com' is an example of such a
	 * sequence.
	 *
	 * @param i
	 *            The index to check
	 ...
	 *
	 * @param possibleEnding
	 *            The possible ending to the String
	 ...
	 *
	 * @param setString
	 *            The string to use as the buffer
	 */
	 ...
	 *
	 * @param setString
	 *            The stream to use as the buffer
	 */
	...
	/**
	 * Removes plurals (-s), and stems of -ed or -ing. Transforms "caresses"
	 * into "caress"-, "ponies" into "poni"-, and "meetings" into "meet"-.
	 * Operates in-place.
	 */
	private void step1() {
		...
		if (ends("eed")) {
			...
			if (ends("at"))
				...
				if (character == 'l' || character == 's' || character == 'z')
					this.wordEndIndex++;
			} else if (countConsonantsInStem() == 1
					&& isConsonantVowelConsonant(this.wordEndIndex))
				setTo("e");
		}
	}
	...
	private void step3() {
		...
		default:
			break;
		// throw new UnexpectedDannError("Unknown suffix from " +
		// buffer[wordEndIndex - 1]);
		}
	}
	...
	private void step5() {
		...
		case 'o':
			if (ends("ion")
					&& this.stemEndIndex >= 0
					&& (this.buffer[this.stemEndIndex] == 's' || this.buffer[this.stemEndIndex] == 't'))
				break;
