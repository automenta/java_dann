...
public final class ArchitectureParse {
    ...
    public static ArchitectureLayer parseLayer(final String line,
        ...
        if ("?".equals(check)) {
            if (defaultValue < 0) {
                throw new RuntimeException("Default (?) in an invalid location.");
            } else {
        ...
        // function
        if (endIndex == -1) {
            throw new RuntimeException("Illegal parentheses.");
        }
    ...
    public static Map<String, String> parseParams(final String line) {
        ...
        while (!parser.eol()) {
            ...
            if (!parser.lookAhead("=", false)) {
                throw new RuntimeException("Missing equals(=) operator.");
            } else {
                parser.advance();
            }
