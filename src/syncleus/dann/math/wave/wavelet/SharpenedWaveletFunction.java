...
import syncleus.dann.math.wave.SharpenedWaveFunction;
...
public class SharpenedWaveletFunction extends SharpenedWaveFunction {
    ...
    protected SharpenedWaveletFunction(final String[] additionalParameters) {
        super(combineLabels(new String[]{"center", "distribution"},
    ...
    private double calculateDistribution() {
        return (1 / (this.getDistribution() * Math.sqrt(2 * Math.PI)))
                * Math.exp(-1 * (Math.pow((this.getX() - this.getCenter()), 2))
                / (2 * Math.pow(this.getDistribution(), 2)));
    }
    ...
    public String toString(final String xName, final String centerName) {
        return "(1 / (distribution * Math.sqrt(2 * pi))) * e^(-1 * ( (" + xName
                + " - " + centerName + ")^2 ) / (2 * distribution^2))" + " * "
                + super.toString(xName);
    }
}
