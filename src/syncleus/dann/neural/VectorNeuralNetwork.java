...
import syncleus.dann.neural.util.structure.NeuralStructure;
...
public class VectorNeuralNetwork<D extends Data> extends AbstractLearning implements ContainsFlat, MLContext, RegressionLearning<D>, VectorEncodable, MLResettable, Classifying<D, Integer>, ErrorLearning<D>, MLFactory, Cloneable {
     ...
     */
    public void addWeight(final int fromLayer, final int fromNeuron,
    ...
    public int calculateNeuronCount() {
        int result = 0;
        result = this.structure.getLayers().stream().map(Layer::getNeuronCount).reduce(result, Integer::sum);
        return result;
    }
     ...
     */
    public double getWeight(final int fromLayer, final int fromNeuron,
                            final int toNeuron) {
        this.structure.requireFlat();
    ...
    public boolean isLayerBiased(final int l) {
        this.structure.requireFlat();
        final int layerNumber = getLayerCount() - l - 1;
        return this.structure.getFlat().getLayerCounts()[layerNumber] != this.structure
    ...
    public void reset(final int seed) {
        final Randomizer randomizer = getRandomizer();
    ...
    public void setBiasActivation(final double activation) {
        ...
        if (this.structure.getFlat() == null) {
            this.structure.getLayers().stream().filter((layer) -> (layer.hasBias())).forEach((layer) -> layer.setBiasActivation(activation));
        } else {
