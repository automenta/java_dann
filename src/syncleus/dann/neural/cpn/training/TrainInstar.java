...
public class TrainInstar extends BasicTraining implements LearningRate {
     ...
     *
     * @param theNetwork      The network to be trained.
     * @param theTraining     The training data.
     * @param theLearningRate The learning rate.
     * @param theInitWeights  True, if the weights should be initialized from the training
        ...
        for (final DataCase pair : this.training) {
            ...
            for (int i = 0; i < pair.getInput().size(); i++) {
                final double diff = pair.getInput().getData(i)
                        - this.network.getWeightsInputToInstar().getNumber(i, winner);
                distance += diff * diff;
            }
            ...
            for (int j = 0; j < this.network.getInputCount(); j++) {
                final double delta = this.learningRate
                        * (pair.getInput().getData(j) - this.network
                        .getWeightsInputToInstar().getNumber(j, winner));
