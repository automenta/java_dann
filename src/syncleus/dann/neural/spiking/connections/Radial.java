...
public class Radial extends Sparse {
     ...
     * @return
     */
    private static double calcConnectProb(SpikingNeuron src, SpikingNeuron tar,
     ...
     * @return
     */
    private static double getRawDist(SpikingNeuron n1, SpikingNeuron n2) {
    ...
    public class DensityEstimator implements Runnable {
        ...
        public void run() {
            int count = 0;
            for (SpikingNeuron src : synapseGroup.getSourceNeurons()) {
                for (SpikingNeuron tar : synapseGroup.getTargetNeurons()) {
                    double randVal = Math.random();
                    double probability;
                    if (src.getPolarity() == Polarity.EXCITATORY) {
                        if (tar.getPolarity() == Polarity.EXCITATORY) {
