...
public class SOMGroup extends NeuronGroup {
     ...
     */
    public SOMGroup(final SpikingNeuralNetwork root, final int numNeurons) {
        super(root);
    ...
    public void update() {
        ...
        for (int i = 0; i < getNeuronList().size(); i++) {
            SpikingNeuron n = getNeuronList().get(i);
            if (n == winner) {
                n.setActivation(1);
            } else {
                n.setActivation(0);
        ...
        // neuron.
        for (int i = 0; i < getNeuronList().size(); i++) {
            SpikingNeuron neuron = getNeuronList().get(i);
     ...
     * @return winner
     */
    private SpikingNeuron calculateWinner() {
        SpikingNeuron winner = null;
        for (int i = 0; i < getNeuronList().size(); i++) {
            SpikingNeuron n = getNeuronList().get(i);
     ...
     */
    private double findPhysicalDistance(final SpikingNeuron neuron1,
            final SpikingNeuron neuron2) {
     ...
     */
    private SpikingNeuron getDefaultSOMNeuron() {
        LinearRule rule = new LinearRule();
        SpikingNeuron ret = new SpikingNeuron(getParentNetwork(), rule);
        rule.setIncrement(1);
