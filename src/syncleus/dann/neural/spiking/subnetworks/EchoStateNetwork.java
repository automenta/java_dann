...
import syncleus.dann.neural.spiking.trainers.LMSOffline;
import syncleus.dann.neural.spiking.trainers.Trainable;
import syncleus.dann.neural.spiking.trainers.Trainer;
import syncleus.dann.neural.spiking.trainers.TrainerListener;
...
public class EchoStateNetwork extends Subnetwork {
     ...
     *            number of output nodes
     */
    public EchoStateNetwork(final SpikingNeuralNetwork network, int inputNodes,
     ...
     */
    public EchoStateNetwork(final SpikingNeuralNetwork network, Point2D initialPosition) {
        super(network);
        this.initialPosition = initialPosition;
        setLabel("Echo-State Network");
    }
    ...
    public void buildNetwork() {
        ...
        List<SpikingNeuron> inputLayerNeurons = initializeLayer(new LinearRule(),
            numInputs);
        List<SpikingNeuron> reservoirLayerNeurons = initializeLayer(
            reservoirNeuronType, numResNodes);
        List<SpikingNeuron> outputLayerNeurons = initializeLayer(new LinearRule(),
            numOutputs);
     ...
     */
    private ArrayList<SpikingNeuron> initializeLayer(NeuronUpdateRule nodeType,
        int nodes) {
        ArrayList<SpikingNeuron> layer = new ArrayList<SpikingNeuron>(nodes);
    ...
    public Trainer getTrainer() {
        ...
        final ArrayList<SpikingNeuron> full = new ArrayList<SpikingNeuron>();
        if (directInOutWeights) {
            for (SpikingNeuron node : getInputLayer().getNeuronList()) {
                full.add(node);
            }
        }
        for (SpikingNeuron node : reservoirLayer.getNeuronList()) {
            full.add(node);
        }
        if (recurrentOutWeights) {
            for (SpikingNeuron node : this.getOutputLayer().getNeuronList()) {
                full.add(node);
            }
        }
        ...
        for (SpikingNeuron n : outputLayer.getNeuronList()) {
    ...
    private double[][] harvestData() {
        ...
        for (int t = 0; t < 2; t++) {
            ...
            for (int row = 0; row < inputData.length; row++) {
                ...
                for (SpikingNeuron neuron : getInputLayer().getNeuronList()) {
                ...
                if (backWeights) {
                    ...
                    for (SpikingNeuron neuron : getOutputLayer().getNeuronList()) {
                        // Teacher forcing
                        if (row > 0) {
                ...
                for (SpikingNeuron n : getReservoirLayer().getNeuronList()) {
                    n.update();
                }
                for (SpikingNeuron n : getReservoirLayer().getNeuronList()) {
