...
public class BackpropTrainer extends IterableTrainer {
    ...
    private List<List<SpikingNeuron>> layers;
     ...
     *            the layers to train
     */
    public BackpropTrainer(Trainable network, List<List<SpikingNeuron>> layers) {
        super(network);
        this.layers = layers;
    ...
    public void apply() {
        ...
        for (int row = 0; row < numRows; row++) {
            ...
            // Update biases
            for (SpikingNeuron neuron : biasDeltaMap.keySet()) {
    ...
    private void backpropagateError(Trainable network, int row) {
        ...
        for (int i = layers.size() - 1; i > 0; i--) {
            ...
            List<SpikingNeuron> layer = layers.get(i);

            // Special update for output layer
            if (i == layers.size() - 1) {
                for (int j = 0; j < numOutputs; j++) {
                    SpikingNeuron outputNeuron = network.getOutputNeurons().get(j);
                ...
                for (SpikingNeuron hiddenLayerNeuron : layer) {
     ...
     */
    private void storeErrorAndDeltas(SpikingNeuron neuron, double error) {
     ...
     */
    protected void randomize(List<SpikingNeuron> layer) {
        for (SpikingNeuron neuron : layer) {
    ...
    protected void updateNetwork() {
        for (List<SpikingNeuron> layer : layers) {
            if (layers.indexOf(layer) != 0) {
                SpikingNeuralNetwork.updateNeurons(layer);
            }
        }
    }
