...
//    private HashMap<NeuronGroupPair, SynapseGroup> routingMap = new HashMap<NeuronGroupPair, SynapseGroup>();
//
//    /** A mapping of neuron groups to unified incoming synapse groups. */
//    private HashMap<NeuronGroup, SynapseGroup> incomingUnified = new HashMap<NeuronGroup, SynapseGroup>();
//
//    /** A mapping of neuron groups to unified outgoing synapse groups. */
//    private HashMap<NeuronGroup, SynapseGroup> outgoingUnified = new HashMap<NeuronGroup, SynapseGroup>();
//
...
//     */
//    public void associateSynapseGroupWithNeuronGroupPair(
//            NeuronGroup sourceNeuronGroup, NeuronGroup targetNeuronGroup,
//            SynapseGroup con) {
//        // Any values can be null. For instance if one wanted to register a
//        // neuron group pair with no connecting synapse group (perhaps awaiting
//        // a
//        // connecting group), the synapse group would simply be passed in as
//        // null. Likewise, if sourceNeuronGroup or targetNeuronGroup are null it
//        // would indicate that the source
//        // or target neurons were loose neurons respectively.
//        NeuronGroupPair gp = new NeuronGroupPair(sourceNeuronGroup,
//                targetNeuronGroup);
//        routingMap.put(gp, con);
//    }
//
//    /**
//     * Any synapse originating in this neuron group is routed to the specified
//     * synapse group.
//     *
//     * @param sourceNeuronGroup the source neuron group
//     * @param con the synapse group to route to
//     */
//    public void associateSynapseGroupWithSourceNeuronGroup(
//            NeuronGroup sourceNeuronGroup, SynapseGroup con) {
//        associateSynapseGroupWithNeuronGroupPair(sourceNeuronGroup, null, con);
//    }
//
//    /**
//     * Any synapse terminating in this neuron group is routed to the specified
//     * synapse group.
//     *
//     * @param sourceNeuronGroup the target neuron group
//     * @param con the synapse group to route to
//     */
//    public void associateSynapseGroupWithTargetNeuronGroup(
//            NeuronGroup targetNeuronGroup, SynapseGroup con) {
...
//     * regardless of its source it will be added to the same SynapseGroup. This
//     * function is quite expensive, which is why it is expected to be used
//     * infrequently.
//     *
//     * @param targetNeuronGroup the target layer for which all incoming synapses
//     *            are being unified into the same group.
//     */
...
//                if (current != null) {
//                    if (ngp.getSourceGroup().getParentGroup() instanceof Subnetwork) {
//                        Subnetwork sourceNet = (Subnetwork) ngp
//                                .getSourceGroup().getParentGroup();
...
//
//    /**
//     * Unifies all the outgoing SynapseGroups from a source NeuronGroup. That
//     * is, if a synapse is added which has @param sourceNeuronGroup as a source
//     * group, then regardless of its target it will be added to the same
//     * SynapseGroup. This function is quite expensive, which is why it is
//     * expected to be used infrequently.
//     *
//     * @param sourceNeuronGroup the target layer for which all incoming synapses
//     *            are being unified into the same group.
//     */
...
//                SynapseGroup current = routingMap.get(ngp);
//                if (current != null) {
//                    if (ngp.getTargetGroup().getParentGroup() instanceof Subnetwork) {
...
//
//    /**
//     * Takes a target neuron group with a unified incoming synapse group and
//     * disjoins that group such that each connected source-target pair is
//     * connected by a different synapse group.
//     *
...
//     * that group such that each connected source-target pair is connected by a
//     * different synapse group.
//     *
...
//        NeuronGroup sourceNeuronGroup = (NeuronGroup) syn.getSource()
//                .getParentGroup();
//        NeuronGroup targetNeuronGroup = (NeuronGroup) syn.getTarget()
//                .getParentGroup();
//
...
//        }
//
//        NeuronGroupPair ngp = new NeuronGroupPair(sourceNeuronGroup,
//                targetNeuronGroup);
...
//
//    /**
//     * Helper method for placing a synapse in a synapse group, and firing the
...
//            // appropriate
//            // PNode
//            synapseGroup.getParentNetwork().transferSynapseToGroup(synapse,
//                    synapseGroup);
...
//            synapseGroup.getParentNetwork().fireGroupChanged(event,
//                    "synapseAddedToGroup");
//        }
//
//    }
//
//    /**
//     * Representation of a pair of neuron groups, which serve as a key for the
//     * routing map, which associates pairs of neuron groups with synapse groups.
//     */
...
//
//        /**
//         * An odd prime number for generating hash codes based on the seed and
//         * hash codes of the internal fields.
//         */
...
//         */
//        public NeuronGroupPair(NeuronGroup sourceNeuronGroup,
//                NeuronGroup targetNeuronGroup) {
//            this.sourceNeuronGroup = sourceNeuronGroup;
//            this.targetNeuronGroup = targetNeuronGroup;
//        }
//
//        /**
//         * Overrides the equals method such that any neuron group pair with the
//         * same neuron groups is considered equal.
//         */
...
//         * source and target neuron group (as well as which one is
//         * sourceNeuronGroup vs. targetNeuronGroup).
//         */
//        public int hashCode() {
...
//
//        boolean test3 = srm.routingMap.containsKey(srm.new NeuronGroupPair(ng1,
//                ng2));
//
//        boolean test4 = srm.routingMap.containsKey(srm.new NeuronGroupPair(ng2,
//                ng1));
//
