...
public class PersistNEATPopulation implements EncogPersistor {
            ...
            if (section.getSectionName().equals("NEAT-POPULATION")
                ...
                for (final String line : section.getLines()) {
                    ...
                    if (cols.get(0).equalsIgnoreCase("s")) {
                        ...
                        final EncogActivationFunction af = EncogFileSection
                                .parseEncogActivationFunction(cols.get(2));
                        neuronGene.setEncogActivationFunction(af);
                ...
                if (afStr.equalsIgnoreCase(PersistNEATPopulation.TYPE_CPPN)) {
                    HyperNEATGenome.buildCPPNEncogActivationFunctions(result
                            .getEncogActivationFunctions());
                } else {
                    result.setNEATEncogActivationFunction(EncogFileSection
                            .parseEncogActivationFunction(params,
    ...
    public void save(final OutputStream os, final Object obj) {
        ...
        if (pop.isHyperNEAT()) {
            ...
            final EncogActivationFunction af = pop.getEncogActivationFunctions()
        ...
        pop.getSpecies().stream().filter((species) -> (species != bestSpecies)).forEach((species) -> saveSpecies(out, species));
        out.flush();
    }
    ...
    private static void saveSpecies(final EncogWriteHelper out, final Species species) {
        ...
        species.getMembers().stream().map((genome) -> (NEATGenome) genome).map((neatGenome) -> {
            ...
            neatGenome
                ...
                out.addColumn(neatNeuronGene.getEncogActivationFunction());
                return neatNeuronGene;
            }).map((neatNeuronGene) -> {
