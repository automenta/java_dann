 ...
 */
package syncleus.dann.neural.train.pso;
...
public class NeuralPSO extends AbstractTraining {
    ...
    private static final boolean m_pseudoAsynchronousUpdate = false;

    /**
     * Constructor.
     *
     * @param network        an initialised Encog network. The networks in the swarm will
     *                       be created with the same topology as this network.
     * @param randomizer     any type of Encog network weight initialisation object.
     ...
     *
     * @param network     The network to train. an initialised Encog network. The
    ...
    void initPopulation() {
        if (m_velocities == null) {
            final int dimensionality = m_bestVector.length;
     ...
     */
    protected void iterationPSO(final boolean init) {
        final TaskGroup group = EngineConcurrency.getInstance()
                .createTaskGroup();

        for (int i = 0; i < m_populationSize; i++) {
            final NeuralPSOWorker worker = new NeuralPSOWorker(this, i, init);
     ...
     */
    protected void updateParticle(final int particleIndex, final boolean init) {
        final int i = particleIndex;
     ...
     */
    protected void updateVelocity(final int particleIndex,
                                  final double[] particlePosition) {
        final int i = particleIndex;
        final double[] vtmp = new double[particlePosition.length];
        ...
        if (i != m_bestVectorIndex) {
            m_va.copy(
                    vtmp,
                    m_pseudoAsynchronousUpdate ? m_bestVectors[m_bestVectorIndex]
                            : m_bestVector);
            m_va.sub(vtmp, particlePosition);
     ...
     * @param particlePosition the particle current position vector
     */
    protected void updatePersonalBestPosition(final int particleIndex,
                                              final double[] particlePosition) {
        // set the network weights and biases from the vector
        final double score = m_calculateScore
                .calculateScore(m_networks[particleIndex]);

        // update the best vectors (g and i)
        if ((m_bestErrors[particleIndex] == 0)
                || isScoreBetter(score, m_bestErrors[particleIndex])) {
     ...
     */
    boolean isScoreBetter(final double score1, final double score2) {
        return ((m_calculateScore.shouldMinimize() && (score1 < score2)) || ((!m_calculateScore
                .shouldMinimize()) && (score1 > score2)));
    }
     ...
     */
    protected double[] getNetworkState(final int particleIndex) {
     ...
     */
    protected void setNetworkState(final int particleIndex, final double[] state) {
    ...
    public String getDescription() {
        return String
                .format("pop = %d, w = %.2f, c1 = %.2f, c2 = %.2f, Xmax = %.2f, Vmax = %.2f",
                        m_populationSize, m_inertiaWeight, m_c1, m_c2,
                        m_maxPosition, m_maxVelocity);
    }
