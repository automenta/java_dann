...
import syncleus.dann.neural.svm.svm;
import syncleus.dann.neural.svm.svm_parameter;
 ...
 */
public class SVMTrain extends AbstractTraining {
     ...
     * Set the constant C.
     *
     * @param theC The constant C.
     */
    public void setC(final double theC) {
        this.c = theC;

        if (this.c <= 0 || this.c < EncogMath.DEFAULT_EPSILON) {
    ...
    public void setGamma(final double theGamma) {
        ...
        if (this.gamma <= 0 || this.gamma < EncogMath.DEFAULT_EPSILON) {
            throw new RuntimeException(
