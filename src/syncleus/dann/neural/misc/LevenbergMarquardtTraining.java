...
import syncleus.dann.data.matrix.hessian.ComputeHessian;
import syncleus.dann.data.matrix.hessian.HessianCR;
 ...
 * http://crsouza.blogspot.
 * com/2009/11/neural-network-learning-by-levenberg_18.html
...
public class LevenbergMarquardtTraining extends AbstractTraining implements
     ...
     */
    private final ComputeHessian hessian;
    ...
    public LevenbergMarquardtTraining(final VectorNeuralNetwork network,
        ...
        final VectorData input = new VectorData(
                this.indexableTraining.getInputSize());
        final VectorData ideal = new VectorData(
                this.indexableTraining.getIdealSize());
        this.pair = new VectorCase(input, ideal);

        this.hessian = h;
    }

    private void saveDiagonal() {
        final double[][] h = this.hessian.getHessian();
    ...
    private double calculateError() {
        final ErrorCalculation result = new ErrorCalculation();
    ...
    public void iteration() {
        ...
        LuDecomposition2 decomposition = null;
        preIteration();
        ...
        while (!done) {
            applyLambda();
            decomposition = new LuDecomposition2(this.hessian.getHessianMatrix());
    ...
    public void setThreadCount(final int numThreads) {
        if (this.hessian instanceof MultiThreadable) {
            ...
            throw new TrainingError("The Hessian object in use("
                    + this.hessian.getClass().toString()
