...
public class GridLayout implements Layout {
    ...
    public void layoutNeurons(final List<SpikingNeuron> neurons) {
        ...
        for (int i = 0; i < neurons.size(); i++) {
            SpikingNeuron neuron = neurons.get(i);
            if (i % numCols == 0) {
                rowNum++;
            }
     ...
     */
    public static List<List<SpikingNeuron>> getColumnList(final List<SpikingNeuron> neurons,
            int numRows) {
        // Sort by x value
        ArrayList<SpikingNeuron> neuronList = new ArrayList<SpikingNeuron>(neurons);
        Collections.sort(neuronList, new Comparator<SpikingNeuron>() {
            public int compare(SpikingNeuron n1, SpikingNeuron n2) {
                return Double.compare(n1.getX(), n2.getX());
            }
        });

        List<List<SpikingNeuron>> grid = new ArrayList<List<SpikingNeuron>>();
        List<SpikingNeuron> currentColumn = new ArrayList<SpikingNeuron>();
        for (int i = 1; i < neuronList.size() + 1; i++) {
            SpikingNeuron neuron = neuronList.get(i - 1);
            currentColumn.add(neuron);
            if ((i > 0) && (i % numRows) == 0) {
                // Sort by y value
                Collections.sort(currentColumn, new Comparator<SpikingNeuron>() {
                    public int compare(SpikingNeuron n1, SpikingNeuron n2) {
                        return Double.compare(n1.getY(), n2.getY());
                    }
                });
                grid.add(currentColumn);
                currentColumn = new ArrayList<SpikingNeuron>();
            }
        }
        return grid;
    }
