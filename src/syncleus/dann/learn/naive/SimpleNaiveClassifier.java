 ...
 *                                                                             *
 ******************************************************************************/
package syncleus.dann.learn.naive;
...
/**
 * A SimpleNaiveClassifier is a simple implementation of a
 * TrainableNaiveClassifier.
 *
...
public class SimpleNaiveClassifier<I, F, C> implements TrainableNaiveClassifier<I, F, C> {
     ...
     */
    @Override
    public C classify(final I item) {
    ...
    public Map<C, Double> getCategoryProbabilities(final I item) {
        final Set<F> features = this.extractor.getFeatures(item);
        final Map<C, Double> categoryProbabilities = new HashMap<>();
        features.stream().map(this::featureClassification).forEach((currentCategory) -> {
            Double newProbability = categoryProbabilities.get(currentCategory);
            if (newProbability == null)
                newProbability = 1.0;
            else
    ...
    public C featureClassification(final F feature) {
        ...
        for (final C category : this.getCategories()) {
            final double currentProbability = this
                    .featureClassificationProbability(feature, category);
    ...
    /**
     * Gets the most likely category for a given feature using a weighted
     * classification.
     *
     * @param feature The feature to use
    ...
    public C featureClassificationWeighted(final F feature) {
        ...
        for (final C category : this.getCategories()) {
            final double currentProbability = this
                    .featureClassificationWeightedProbability(feature, category);
            if (topProbability < currentProbability) {
                topCategory = category;
                topProbability = currentProbability;
            }
    ...
    public double featureClassificationProbability(final F feature,
                                                   final C category) {
        final double overallProb = this.getOverallProbability(category);
        double featureProb = 0;
        if (this.featureTree.containsKey(feature))
            featureProb = this.featureTree.getFeature(feature)
     ...
     */
    @Override
    public double featureClassificationWeightedProbability(final F feature,
                                                           final C category) {
        final double unweightedProb = this.featureClassificationProbability(
                feature, category);
        double total = 0.0;
        if (this.featureTree.containsKey(feature))
    ...
    public Set<C> getCategories() {
        return Collections.unmodifiableSet(this.overallCategoryProbability
                .getCategoryProbabilityMap().keySet());
    }
    ...
    public void train(final I item, final C category) {
        final Set<F> features = this.extractor.getFeatures(item);
        features.stream().forEach((feature) -> this.featureTree.getFeature(feature).incrementCategory(category));
