...
/**
 * An AbstractDirectedAdjacencyGraph is a DirectedGraph implemented using
 * adjacency lists.
 *
 ...
 * @since 2.0
 */
public abstract class AbstractDirectedAdjacencyGraph<N, E extends DirectedEdge<N>>
     ...
     */
    protected AbstractDirectedAdjacencyGraph(final Set<N> nodes,
                                             final Set<E> edges) {
        super(nodes, edges);
    }
    ...
    @Override
    public AbstractDirectedAdjacencyGraph<N, E> cloneAdd(final Set<N> newNodes,
                                                         final Set<E> newEdges) {
        return (AbstractDirectedAdjacencyGraph<N, E>) super.cloneAdd(newNodes,
                newEdges);
    }

    @Override
    public AbstractDirectedAdjacencyGraph<N, E> cloneRemove(final E edgeToRemove) {
        return (AbstractDirectedAdjacencyGraph<N, E>) super
    ...
    @Override
    public AbstractDirectedAdjacencyGraph<N, E> cloneRemove(
            final Set<N> deleteNodes, final Set<E> deleteEdges) {
        return (AbstractDirectedAdjacencyGraph<N, E>) super.cloneRemove(
    ...
    public Iterator<E> iterateAdjacentEdges(final N source, final N target) {
        final Set<E> ec = getAdjacentEdges(source);
        if (ec == null)
            return Collections.EMPTY_LIST.iterator();

        return ec.stream().filter(e -> e.getDestinationNode() == target)
                .iterator();
    }
