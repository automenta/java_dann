...
public abstract class AbstractHyperEdge<N> extends AbstractEdge<N> implements
        HyperEdge<N> {
    ...
    protected AbstractHyperEdge(final boolean allowJoiningMultipleGraphs,
    ...
    public List<N> getTraversableNodes(final N node) {
        ...
        if (!traversableNodes.remove(node))
            throw new IllegalArgumentException(
                    "node is not one of the end points!");
    ...
    public AbstractHyperEdge<N> disconnect(final N node) {
        ...
        if (!this.getNodes().contains(node))
            throw new IllegalArgumentException(
                    "node is not currently connected to");
