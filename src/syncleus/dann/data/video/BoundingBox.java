...
public class BoundingBox extends Rect{
	...
	public float calcOverlap(final Rect other){
	...
	public Point[] points(){
		...
		System.out.println("Points in BB: " + this + " stepx=" + stepx + " stepy=" + stepy + " RES size=" + result.size());			
	...
	BoundingBox predict(final Point[] points1, final Point[] points2){
		...
		System.out.println("Tracked points: " + npoints);
		...
		final float dx = TLDUtil.median(xoff);
		final float dy = TLDUtil.median(yoff);
		...
		if(npoints > 1){
			...
			for(int i = 0; i < npoints; i++){
				for(int j = i + 1; j < npoints; j++){
					d[idx++] = TLDUtil.norm(points2[i], points2[j]) / TLDUtil.norm(points1[i], points1[j]);
				}
			}
			s = TLDUtil.median(d);
		}
		...
		System.out.println("Current BB: " + this + ", Predicted BB: " + result);
