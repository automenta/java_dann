...
class LKTracker {
	...
	private Pair<Point[], Point[]> filterPts(final Point[] lastPoints, final Point[] currentPoints, final float[] similarity, final float[] errFB, final byte[] status){
		...
		final float similarityMed = TLDUtil.median(similarity);
		System.out.println("Filter points MED SIMILARITY: " + similarityMed);
		...
		if(filteredErrFB.size() > 0){
			errFBMed = TLDUtil.median(filteredErrFB);
			...
			System.out.println("Filter points MED ErrFB: " + errFBMed + " K count=" + filteredLastPoints2.size());
		}
