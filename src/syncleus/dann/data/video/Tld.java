...
public class Tld {
	...
	public void init(Mat frame1, Rect trackedBox) {
		...
		// TRAINING
		System.out.println("Init Start Training with " + fernsData.size() + " ferns, " 
	...
	private void updateIntegralImgs(Mat frame) {
		...
		_iisumJava = Arrays.copyOf(TLDUtil.getIntArray(_iisum), _iiRows * _iiCols);
		_iisqsumJava = Arrays.copyOf(TLDUtil.getDoubleArray(_iisqsum), _iiRows * _iiCols);
	}
	...
	public ProcessFrameStruct processFrame(final Mat lastImg, final Mat currentImg){
		...
		if(trackingStruct != null){
			...
			if(trackingStruct.conf > _classifierNN.getNNThresholdValid()){
				System.out.println("Tracking confidence: " + trackingStruct.conf + " > " + " Threshold: " + _classifierNN.getNNThresholdValid() + " ===> WILL LEARN");
				_learn = true;
			}else{
				System.out.println("Tracking confidence: " + trackingStruct.conf + " < " + " Threshold: " + _classifierNN.getNNThresholdValid() + " ===> WILL NOT LEARN");
			}
			...
			if(detStructs != null){
				...
				if(confidentClusters.size() == 0){
					System.out.println("NO NN confident cluster !");
				}else if(confidentClusters.size() == 1){
					System.out.println("Detected better match (1 confident cluster), re-initialising tracker");
					_lastbox = confidentClusters.keySet().iterator().next(); //bbnext
					_learn = false;
				}else{
					System.out.println("Plenty of confident clusters detected. Get mean of close detections (use nnMatches)");
	...
	private TrackingStruct track(final Mat lastImg, final Mat currentImg, final BoundingBox lastBox) {
		...
		if(_tracker.getMedianErrFB() > _params.tracker_stability_FBerrMax){
			System.out.println("TRACKER too unstable. FB Median error: " + _tracker.getMedianErrFB() + " > " + _params.tracker_stability_FBerrMax);
	...
	private Pair<List<DetectionStruct>, List<DetectionStruct>> detect(final Mat frame){
		...
		for(DetectionStruct detStruct : fernClassDetected){
			...
			System.out.println("NNConf: " + detStruct.nnConf.relativeSimilarity + " / " + detStruct.nnConf.conservativeSimilarity + " Threshold: " + _classifierNN.getNNThreshold());
	...
	private Map<BoundingBox, Float> clusterConfidentIndices(final List<DetectionStruct> conservativeSimilarities){
		...
		if(numbb == 0){
			System.out.println("NO conservative similarities provided, NOTHING to cluster.");
	...
	private Pair<List<Pair<int[], Boolean>>, List<Mat>> generateNegativeData(final Mat frame){
		...
		System.out.println("Negative examples generated. Ferns count: " + negFerns.size() + ". negEx count: " + negExamples.size());
