...
public class FunctionFactory implements Serializable {
     ...
     *
     * @param name    The name of the opcode.
     * @param program The program to factor for.
    ...
    public List<ProgramExtensionTemplate> findOpcodes(
        ...
        this.opcodes.stream().forEach((temp) -> types.stream().filter((rtn) -> (temp.isPossibleReturnType(context, rtn))).forEach((_item) -> {
    ...
    public ProgramExtensionTemplate findOperator(final char ch1, final char ch2) {
        ...
        if (ch2 == 0) {
            return findOperatorExact(String.valueOf(ch1));
        }

        // first, see if we can match an operator with both ch1 and ch2
        result = findOperatorExact(String.valueOf(ch1) + ch2);

        if (result == null) {
            // okay no 2-char operators matched, so see if we can find a single
            // char
            result = findOperatorExact(String.valueOf(ch1));
        }
     ...
     *
     * @param theOpCode The opcode index.
     * @return The opcode.
     */
     ...
     *
     * @param name The name of the opcode.
     * @param l    The argument count for the opcode.
