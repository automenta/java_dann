...
public class SignalProcessingWavelet implements
    ...
    // </editor-fold>
    private long id;
    private final SignalConcentration output;
    ...
    public SortedSet<SignalConcentration> getSignals() {
        final SortedSet<SignalConcentration> copy = new TreeSet<>(
                this.signals);
    ...
    public void preTick() {
        this.reconstructWavelet();
        this.signals.stream().forEach((signal) -> this.wavelet.setParameter(this.wavelet.getParameterNameIndex(String
    ...
    public SignalProcessingWavelet mutate(final double deviation) {
        ...
        do {
            ...
            else if (roll < waveGenerationChance) {
                copy.waves.add(this.generateNewWave());
                copy.id = RANDOM.nextLong();
            }
            // 10% to delete a RANDOM wave
            else if (roll < waveDeletionChance) {
        ...
        } while (RANDOM.nextFloat() < exitChance);
        return copy;
    }
