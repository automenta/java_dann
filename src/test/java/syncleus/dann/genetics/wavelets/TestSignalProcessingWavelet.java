...
public class TestSignalProcessingWavelet {
	...
	private static final Logger LOGGER = LogManager
			.getLogger(TestSignalProcessingWavelet.class);
	...
	public void testMutationOnce() {
		...
		SignalProcessingWavelet processor = new SignalProcessingWavelet(xAxis,
				output);
	...
	public void testXorEvolveOnce() {
		...
		while ((population.size() < POPULATION_SIZE)
				&& ((population.isEmpty() ? 0.0 : population.lastKey()) < 4.0)) {
			SignalProcessingWavelet processor = new SignalProcessingWavelet(
					xAxis, output);
			processor = mutateXor(processor, xAxis, yAxis);
			final double initialFitness = checkXorFitness(
					processor.getWavelet(), processor.getWaveCount());
			population.put(initialFitness, processor);
			LOGGER.debug("Initialized " + population.size()
					+ " members, last members fitness: " + initialFitness);
		}
		// run through several generations
		LOGGER.debug("population initalized, proceeding with generations");
		int generationIndex;
		for (generationIndex = 0; (generationIndex < GENERATIONS)
				&& (population.lastKey() < 4.0); generationIndex++) {
			LOGGER.debug("Begining generation " + generationIndex
					+ ", current fitness: " + population.lastKey());
			...
			final ArrayList<SignalProcessingWavelet> populationArray = new ArrayList<SignalProcessingWavelet>(
					population.values());
			while (population.size() < POPULATION_SIZE) {
				SignalProcessingWavelet processor;
				if (RANDOM.nextFloat() < 0.5)
					processor = populationArray.get(RANDOM
							.nextInt(populationArray.size()));
				else
		...
		LOGGER.debug("evolution completed in " + generationIndex
				+ " generations.");
		if (bestFitness < 4.0)
			LOGGER.warn("did not successfully match XOR truth table: fitness: "
					+ bestFitness);
		Assert.assertTrue(
				"did not successfully match XOR truth table: fitness: "
						+ bestFitness, bestFitness >= 4.0);
	}

	private static SignalProcessingWavelet mutateXor(
			final SignalProcessingWavelet processor,
			final GlobalSignalConcentration xAxis,
			final GlobalSignalConcentration yAxis) {
		// mutate until there are atleast 2 input signals and 1 output signal, 3
		// total
	...
	private static double checkXorFitness(final Function xorAttempt,
		...
		// calculates the whole number portion of the fitness, should be between
		// -4 and +4
		final int fitnessWhole = (result00 < 0.0 ? 1 : 0)
				+ (result10 > 0.0 ? 1 : 0) + (result01 > 0.0 ? 1 : 0)
				+ (result11 < 0.0 ? 1 : 0);

		// calculates the decimal portion of the fitness , should be >= 0 and <
		// 1
		final double fitnessFine = 1.0 - Math.tanh(waveCount);
		final double fitnessSuperFine = Math
				.abs((double) xorAttempt.hashCode()) / (Integer.MAX_VALUE);

		return (fitnessWhole) + (fitnessFine * 0.99999)
				+ (fitnessSuperFine * 0.00001);
	}
}
