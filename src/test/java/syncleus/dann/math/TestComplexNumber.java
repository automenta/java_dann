...
public class TestComplexNumber {
	...
	public void testComplexMath() {
		...
		Assert.assertTrue("addition failed: " + valueA + " + " + valueB + " = "
				+ result, result.equals(expected));

		result = valueA.subtract(valueB);
		expected = new ComplexNumber(-15.0, -3.0);
		Assert.assertTrue("subtraction failed: " + valueA + " - " + valueB
				+ " = " + result, result.equals(expected));

		result = valueA.multiply(valueB);
		expected = new ComplexNumber(-66.0, -26.0);
		Assert.assertTrue("multiplication failed: " + valueA + " * " + valueB
				+ " = " + result, result.equals(expected));

		result = valueA.multiply(10.0);
		expected = new ComplexNumber(-80.0, 20.0);
		Assert.assertTrue("multiplication failed: " + valueA + " * " + 10.0
				+ " = " + result, result.equals(expected));

		result = valueA.divide(valueB);
		expected = new ComplexNumber(-0.6216216216216217, 0.7297297297297298);
		Assert.assertTrue("division failed: " + valueA + " / " + valueB + " = "
				+ result, result.subtract(expected).absScalar() < 0.0001);

		result = valueA.conjugate();
		expected = new ComplexNumber(-8.0, -2.0);
		Assert.assertTrue("conjugation failed: conjugate(" + valueA + ") = "
				+ result, result.equals(expected));

		Assert.assertTrue(
				"absolute failed: abs(" + valueA + ") = " + valueA.absScalar(),
				(valueA.absScalar() - 8.246211251235321) < 0.0001);
