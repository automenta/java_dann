...
import syncleus.dann.math.linear.decomposition.Decompositions;
import syncleus.dann.math.linear.decomposition.DoolittleLuDecomposition;
import syncleus.dann.math.linear.decomposition.EigenvalueDecomposition;
import syncleus.dann.math.linear.decomposition.HouseholderQrDecomposition;
import syncleus.dann.math.linear.decomposition.LuDecomposition;
import syncleus.dann.math.linear.decomposition.QrDecomposition;
...
public class TestMagicSquare {
	...
	public void testMagicSquare() {
		...
		for (int n = 3; n <= 32; n++) {
			...
			Assert.assertTrue("incorrect trace! obtained: " + t + " expects: "
			...
			Assert.assertTrue(
					"incorrect maximum eigen! obtained: "
							+ d.get(n - 1).getValue() + " expected: "
							+ solutions[n - 3][0] + " on matrix: "
							+ currentMatrix,
					checkValues(d.get(n - 1).getValue(), solutions[n - 3][0],
							0.01));
			final int r = currentMatrix.rank();
			Assert.assertTrue("incorrect rank!",
					checkValues(r, solutions[n - 3][1]));
			double c = currentMatrix.cond();
			if (c >= 1 / eps)
				c = Double.POSITIVE_INFINITY;
			Assert.assertTrue("incorrect cond!",
					checkValues(c, solutions[n - 3][2]));
			final LuDecomposition<RealMatrix, RealNumber> LU = new DoolittleLuDecomposition<RealMatrix, RealNumber>(
					currentMatrix);
			final RealMatrix L = LU.getLowerTriangularFactor();
			final RealMatrix U = LU.getUpperTriangularFactor();
			final int[] p = LU.getPivot();
			SimpleRealMatrix R = (SimpleRealMatrix) L.multiply(U).subtract(
					currentMatrix.getSubmatrix(p, 0, n - 1));
			double res = R.norm1Double() / (n * eps);
			Assert.assertTrue("incorrect lu_res!",
					checkValues(res, solutions[n - 3][3]));
			final QrDecomposition<RealMatrix, RealNumber> QR = new HouseholderQrDecomposition<RealMatrix, RealNumber>(
					currentMatrix);
			...
			Assert.assertTrue("incorrect qr_res! obtained: " + res
					+ " expects: " + solutions[n - 3][4] + " on matrix: "
					+ currentMatrix, checkValues(res, solutions[n - 3][4]));
		}
	}
}
