...
import syncleus.dann.graph.path.pathfinding.BellmanFordPathFinder;
...
public class TestBellmanFordPathFinder {
	...
	private static boolean checkSolution(final GridNode start,
			final List<DirectedEdge<GridNode>> path, final int[][] solution) {
		int solutionIndex = 0;
	...
	public void testHardGrid() {
		...
		final BellmanFordPathFinder<GridNode, DirectedEdge<GridNode>> pathFinder = new BellmanFordPathFinder<GridNode, DirectedEdge<GridNode>>(
				hardGrid);
		...
		final List<DirectedEdge<GridNode>> path = pathFinder.getBestPath(
				startNode, endNode);
