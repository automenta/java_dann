...
public class WeightedDirectedGrid
		extends
		AbstractBidirectedAdjacencyGraph<GridNode, WeightedDirectedEdge<GridNode>> {
		...
		for (int y = 0; y < nodeWeights.length; y++)
			for (int x = 0; x < nodeWeights[0].length; x++) {
				...
				this.inNeighborEdges.put(this.nodes[y][x],
						new HashSet<WeightedDirectedEdge<GridNode>>());
		...
		for (int y = 0; y < nodes.length; y++)
			for (int x = 0; x < this.nodes[0].length; x++) {
				...
				if (x < this.nodes[0].length - 1) {
					SimpleWeightedDirectedEdge<GridNode> newEdge = new SimpleWeightedDirectedEdge<GridNode>(
							this.nodes[y][x], this.nodes[y][x + 1],
							this.nodes[y][x + 1].getWeight());
					...
					this.inNeighborNodes.get(this.nodes[y][x + 1]).add(
							this.nodes[y][x]);
					newEdge = new SimpleWeightedDirectedEdge<GridNode>(
							this.nodes[y][x + 1], this.nodes[y][x],
							this.nodes[y][x].getWeight());
					this.edges.add(newEdge);
	...
	@Override
	public Stream<GridNode> streamNodes() {
		return nodeSet.stream();
	}

	@Override
	public Stream<WeightedDirectedEdge<GridNode>> streamEdges() {
		return edges.stream();
	}

	@Override
	public Stream<WeightedDirectedEdge<GridNode>> streamAdjacentEdges(
			final GridNode node) {
		return getAdjacentEdges(node).stream();
	}

	@Override
	public Stream<WeightedDirectedEdge<GridNode>> streamTraversableEdges(
			final GridNode node) {
	...
	@Override
	public Set<WeightedDirectedEdge<GridNode>> getAdjacentEdges(
			final GridNode node) {
		final Set<WeightedDirectedEdge<GridNode>> newEdges = new HashSet<WeightedDirectedEdge<GridNode>>(
	...
	@Override
	public Set<WeightedDirectedEdge<GridNode>> getTraversableEdges(
			final GridNode node) {
		final Set<WeightedDirectedEdge<GridNode>> newEdges = new HashSet<WeightedDirectedEdge<GridNode>>(
	...
	public List<GridNode> getAdjacentNodes(final GridNode node) {
		final ArrayList<GridNode> newNeighbors = new ArrayList<GridNode>(
