...
public class InteractiveTests {
	...
	private static final Logger LOGGER = LogManager
			.getLogger(InteractiveTests.class);
	...
	private static class ClassComparator implements Comparator<Class>,
		...
		public boolean equals(final Object compareWith) {
			return compareWith instanceof syncleus.dann.InteractiveTests.ClassComparator;
		}
	...
	private static Map<Class, Set<Method>> getTestPoints() {
		final Map<Class, Set<Method>> testPoints = new TreeMap<Class, Set<Method>>(
				new ClassComparator());
		final Class[] classes;
		try {
		...
		for (final Class packageClass : classes) {
			...
			final String classString = fullClassString
					.substring(classNameIndex);
			if ((!classString.contains("$"))
					&& (InteractiveTests.isTestClass(packageClass))) {
				// check to make sure there is a default constructor
				boolean hasDefaultContructor = false;
				final Constructor[] constructors = packageClass
						.getConstructors();
				...
				final Set<Method> testMethods = new TreeSet<Method>(
						new MethodComparator());
				testPoints.put(packageClass, testMethods);
				final Method[] packageClassMethods = packageClass
						.getDeclaredMethods();
	...
	private static Method selectTest(final Map<Class, Set<Method>> testPoints)
			throws Exception {
		int currentChoice = 1;
		final Map<Integer, Method> choices = new HashMap<Integer, Method>();
		for (final Map.Entry<Class, Set<Method>> classSetEntry : testPoints
				.entrySet()) {
	...
	public static void main(final String[] args) {
		try {
			...
			logProperties.setProperty("log4j.appender.console",
					"org.apache.log4j.ConsoleAppender");
			logProperties.setProperty("log4j.appender.console.Target",
					"System.out");
			logProperties.setProperty("log4j.appender.console.layout",
					"org.apache.log4j.PatternLayout");
			logProperties
					.setProperty("log4j.appender.console.Threshold", "all");
			// PropertyConfigurator.configure(logProperties);

			final Map<Class, Set<Method>> tests = InteractiveTests
					.getTestPoints();
			...
			final Request testRequest = Request.method(testClass,
					test.getName());
