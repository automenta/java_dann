...
public class UtilTest extends OpenCVTestCase {
	...
	public void testKeepBestN(){
		...
		TLDUtil.keepBestN(boxes, 10, new Comparator<BoundingBox>(){
			@Override
	...
	public void testToByteArray(){
		...
		final byte[] array = TLDUtil.getByteArray(greyMat);
		...
		for(int row=0; row<greyMat.rows(); row++){
			for(int col=0; col<cols; col++){
				assertEquals(TLDUtil.getByte(row, col, greyMat), array[row * cols + col]);
			}
		}
	}
	...
	public void testSubmat(){
		...
		assertEquals(2, TLDUtil.getByte(0,  2, greyMat));
		assertEquals(2, TLDUtil.getByte(0,  2, submat));
		
		submat.put(0, 2, new byte[]{33});
		assertEquals(33, TLDUtil.getByte(0,  2, greyMat));
		assertEquals(33, TLDUtil.getByte(0,  2, submat));		
	}
	...
	public void testGetVar(){
		...
		assertEquals("Wrong Var calculation", -417.55555, TLDUtil.getVar(test_box, img, img2), 0.00001);
	}
}
