...
public class TestColor {
    ...
    public void testColor() {
        ...
        final ThreadPoolExecutor executor = new ThreadPoolExecutor(cores + 1,
                cores * 2, 20, TimeUnit.SECONDS,
                new LinkedBlockingQueue<>());
        try {
            // initialize brain with 3d input and 2d output
            final ExponentialDecaySomBrain brain = new ExponentialDecaySomBrain(
                    INPUT_DIMENSIONS, OUTPUT_DIMENSIONS, TRAIN_ITERATIONS,
                    LEARNING_RATE, executor);
            ...
            for (int iteration = 0; iteration < TEST_ITERATIONS; iteration++) {
                ...
                outText.append("close color offsets... red: ")
                        .append(redOffset).append(", green: ")
                        .append(greenOffset).append(", blue: ")
                        .append(blueOffset).append('\n');

                // get the location of a color within the block
                brain.setInput(0, redOffset + (random.nextDouble() * blockSize));
                brain.setInput(1, greenOffset
                        + (random.nextDouble() * blockSize));
                brain.setInput(2, blueOffset
                        + (random.nextDouble() * blockSize));
                outText.append("close color1... red:")
                        .append(brain.getInput(0)).append(", green: ")
                        .append(brain.getInput(1)).append(", blue")
                        .append(brain.getInput(2)).append('\n');
                ...
                brain.setInput(2, blueOffset
                        + (random.nextDouble() * blockSize));
                outText.append("close color2... red:")
                        .append(brain.getInput(0)).append(", green: ")
                        .append(brain.getInput(1)).append(", blue")
                        .append(brain.getInput(2)).append('\n');
                ...
                outText.append("close color1 point: ").append(color1)
                        .append('\n');
                outText.append("close color2 point: ").append(color2)
                        .append('\n');
                final double distance = color1.calculateRelativeTo(color2)
                        .getDistance();
                outText.append("close color distance: ").append(distance)
                        .append('\n');
            ...
            for (int iteration = 0; iteration < TEST_ITERATIONS; iteration++) {
                ...
                brain.setInput(0, (isRed1Positive ? random.nextDouble()
                        * maxDrift : 1.0 - (random.nextDouble() * maxDrift)));
                brain.setInput(1, (isGreen1Positive ? random.nextDouble()
                        * maxDrift : 1.0 - (random.nextDouble() * maxDrift)));
                brain.setInput(2, (isBlue1Positive ? random.nextDouble()
                        * maxDrift : 1.0 - (random.nextDouble() * maxDrift)));
                outText.append("far color1... red:").append(brain.getInput(0))
                        .append(", green: ").append(brain.getInput(1))
                        .append(", blue").append(brain.getInput(2))
                        .append('\n');
                final Vector color1 = brain.getBestMatchingUnit(true);
                ...
                brain.setInput(
                        0,
                        (isRed1Positive ? 1.0 - (random.nextDouble() * maxDrift)
                ...
                brain.setInput(
                        ...
                        (isBlue1Positive ? 1.0 - (random.nextDouble() * maxDrift)
                                : random.nextDouble() * maxDrift));
                outText.append("far color2... red:").append(brain.getInput(0))
                        .append(", green: ").append(brain.getInput(1))
                        .append(", blue").append(brain.getInput(2))
                        .append('\n');
                final Vector color2 = brain.getBestMatchingUnit(true);
                ...
                // calculate the distance between these two points
                outText.append("far color1 point: ").append(color1)
                        .append('\n');
                outText.append("far color2 point: ").append(color2)
                        .append('\n');
                final double distance = color1.calculateRelativeTo(color2)
                        .getDistance();
                outText.append("far color distance: ").append(distance)
                        .append('\n');
                // store the distance if its greater than the current max
            ...
            Assert.assertTrue("colors did not map properly: far: "
                            + closestDistanceFar + " -> close: "
                            + farthestDistanceClose + '\n' + closeOutText + '\n'
                            + farOutText + '\n',
